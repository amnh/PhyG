...
\label{subsec:refine}
	...
	\begin{phygdescription}
		...
		trees and graphs. Full Simulated Annealing \citep{Metropolisetal1953, Kirkpatricketal1983, Cerny1985}
		and Genetic Algorithm \citep{Holland1975, moilanen1999} are implemented for 
		any graph type. In addition, it performs edit operations (addition, deletion, and move) 
		on network edges that only applies to softwired and hardwired graphs.}
	\end{phygdescription}
	...
	\begin{description}
		...
		Kirkpatricketal1983, Cerny1985}. The acceptance of candidate graphs is 
		...
		annealing optimization \citep{goloboff1999} that are performed. The acceptance of 
		candidate graphs is determined by the probability $1/ (wf + c_c - c_b)$, where $c_c$ 
		is the cost of the candidate graph, $c_b$ is the cost of the current best graph, and $wf$ 
		is the \texttt{acceptWorse} (set by the \texttt{acceptWorse:FLOAT}, default 1.0) option. 
		Equal cost graphs are accepted with probability set by the \texttt{acceptEqual} option. 
...
%			the penalty. The default is 1.0. Not yet implemented.
	...
	\begin{description}
		...
		Note: when deciding this value, the user should be aware that the addition of network 
		edges exponentially increases the time taken to optimize the graph 
		\cite{WheelerandWashburn2023}. This argument is also used in conjunction with \texttt{geneticAlgorithm}.
	...
	\subsubsection{Defaults}
		\noindent \texttt{refine(ga, generations:10, popsize:20, recombinations:100)} By default, 
	  	  \phyg will perform Genetic Algorithm refinement, with its associated default options, 
		   if no other arguments are specified.}\\
		   
		\noindent \texttt{refine(netadd | netdel | netadddel | netmove, steepest, atrandom)} By default, 
	...
	\begin{example}
		...
		\item{\texttt{refine(geneticalgorithm, population:5, generations:100, maxnetedges:5, 
		recombinations:20, annealing:5)} \phyg will perform Genetic Algorithm 
		refinement, with its associated default options, if no other arguments are specified.}\\
		   
		\item{\texttt{refine(ga, generations:100, maxnetedges:5, recombinations:20, annealing:5)} By default, 
	  	 \phyg will perform Genetic Algorithm refinement, with its associated default options, 
		  if no other arguments are specified.}\\
...
\subsection{Rename}
	...
	expressions Coleonyx* and Coleonix* 	will be renamed as Coleonyx\_sp. Irrespective of where this 
...
\label{subsec:search}
	...
	\begin{description}
		...
		terminate only when all instances have completed. \hl{Employing this argument is
...
\label{subsec:support}
	...
	\begin{phygdescription}
		{Calculates the requested support values. \phyg implements support estimation based on 
		resampling methods (Jackknife ~\cite{Farrisetal1996} and Bootstrap~\cite{Felsenstein1985})
		and Goodman-Bremer support~\cite{Goodmanetal1982, Bremer1988}.  The calculation of 
		support measures based on resampling, is done at the fragment or ``block'' level, rather
		than at the nucleotide level. For this reason, these data need to be collated into a block
		with the command \texttt{reblock} (see Section \ref{subsec:reblock}). Goodman-Bremer
		calculations are optimality-based measure based on the cost of the tree, and are not 
