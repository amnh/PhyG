...
%		\item[dynamicepsilon:[FLOAT]]  Only internal---used for testing purposes.
%		Alters the performance of the heuristic graph cost 
...
%		Only internal---used for testing purposes.
		...
		\item[useCurrentGraph[:BOOL]] Specifies that the current graph(s) (e.g searched or input)
			are used instead of building new graphs with the resampled data.  This option turns of
			\texttt{buildOnly} and is the default for \texttt{bootstrap} and \texttt{jackknife}. 
	...
	\subsubsection{Defaults}
		...
		%WCW useCurrentGraph:True is default for jackknife and bootstrap
...
\label{subsec:swap}
	...
	\begin{phygdescription}
		...
		%Default swapping trajectories are randomized.
		}
	\end{phygdescription}
...
%		\item[dynamicEpsilon] Only internal---used for testing purposes.
%		Sets the level at which heuristic graph costs are verified by full 
...
%		Sets graph rejoin heuristic threshold (as described in 
%		\citealp{VaronandWheeler2013}) during SPR and TBR branch swapping (default 1.17). 
%		Higher values will allow more heuristic graph rejoins, resulting in potentially better 
%		solutions at the cost of greater execution times.
