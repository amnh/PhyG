...
\label{subsec:reblock}
	...
	\begin{phygdescription}
		...
		or ``block'' level, rather than at the nucleotide level. For this reason, these data need
		to be collated into a block with \texttt{reblock} (see Section \ref{subsec:support}).} 
	\end{phygdescription}
...
\label{subsec:refine}
	...
	\begin{phygdescription}
		...
		deletion, and move) on network edges that only applies to softwired and hardwired 
		graphs.}
	\end{phygdescription}
	...
	to search for more optimal graphs. The following arguments will mimic this evolutionary 
	process. 
...
%			\item[elitist] Not yet implemented.
			...
			\begin{description}
				...
				generations has been completed. This integer value should be less than that as
				specified in \texttt{generations}.
			
			\end{description}
			
			\item[maxnetedges:INT] Specifies the maximum number of network edges in the 
			graphs to be produced. The number of network edges in the graphs produced 
			by these edit operations are limited by the number as specified by the integer 
			value. Note: when deciding this value, the user should be aware that the addition 
			of network edges exponentially increases the time taken to optimize the graph 
			\cite{WheelerandWashburn2023}. The user must specify a \texttt{softwired} or 
			\texttt{hardwired} graph type for this option to be performed (see Section \ref{subsec:set}). 
			This argument is also used in conjunction with the network edit 	operations \texttt{netadd} 
			and \texttt{netadddel}. 
			...
			other graphs. The default is 20.
...
%			the penalty. The default is 1.0. Not yet implemented.
	...
	\end{description}
		...
		including addition, deletion and movement of network edges.
	...
	\begin{description}
		...
		steps (set by \texttt{steps:INT}, default 10).
		...
		annealing optimization \citep{goloboff1999} that are performed. This is done in 
		concert with \texttt{netAdd}, \texttt{netDel}, and \texttt{netMove}. The acceptance 
		...
		and $wf$ is the \texttt{acceptWorse} (set by the \texttt{acceptWorse:FLOAT}, default 
		1.0) option. Equal cost graphs are accepted with probability set by the \texttt{acceptEqual} 
		option. \texttt{drift} differs from \texttt{annealing} in that there are no cooling steps 
		to modify acceptance probabilities. The maximum number of graph changes is set by 
		\texttt{maxChanges}.
		
		\item[inorder] Contra \texttt{atRandom}, network edit neighborhoods are always 
		traversed in the same undefined, but consistent order.

		\item[keep:INT] Limits the number of returned graphs to that as specified by the 
		integer value. The default is 10.
		
		\item[maxnetedges:INT] Specifies the maximum number of network edges in the 
		graphs to be produced. This argument is used in conjunction with the network edit 
		operations \texttt{netadd} and \texttt{netadddel}. The number of network edges in 
		the graphs produced by these edit operations are limited by the number as specified 
		by the integer value. Note: when deciding this value, the user should be aware that 
		the addition of network edges exponentially increases the time taken to optimize the 
		graph \cite{WheelerandWashburn2023}. The user must specify \texttt{softwired} or 
		\texttt{hardwired} \texttt{graphtype} for this option to be performed (see Section 
		\ref{subsec:set}). This argument is also used in conjunction with \texttt{geneticAlgorithm}.
	...
	\subsubsection{Defaults}
		\noindent \texttt{refine(ga, generations:10, popsize:20, recombinations:100)} By 
		default, \phyg will perform Genetic Algorithm refinement, with its associated default 
		options, if no other arguments are specified.}\\
		   
		\noindent \texttt{refine(netadd | netdel | netadddel | netmove, steepest, atrandom)} 
		By default, during network edit operations, \phyg will follow a greedy path and 
	...
	\begin{example}
		...
		\item{\texttt{refine(netadddel, rounds:3, maxnetedges:5)}\\ Consecutively adds 
		...
		during network edit operations, \phyg will follow a greedy path and traverse network 
		edit neighborhoods in a randomized order.}
		...
		edit neighborhoods are traversed in the same undefined, but consistent order.
		This follows a greedy path, abandoning the neighborhood of the current 
		graph when a better graph of lower cost is found.}
		
		\item{\texttt{refine(netadd, annealing:10, steps:5, acceptWorse:0.5)} \\ 
		Consecutively adds network edges from input graphs until no better cost
		graph is found. XXX. 10 rounds of simulated annealing optimization will be 
		performed, limiting the number of temperature steps to 5. During each round, 
		all graphs XXX will be accepted.}
		
		\item{\texttt{refine(netdel, drift, acceptEqual:2.0)} \\ 
		Consecutively deletes network edges from input graphs until no better cost
		graph is found. XXX  rounds of drifting will be performed. During each round, 
		all graphs XXX will be accepted.}
...
\label{subsec:search}
	...
	\begin{description}
		...
		recommended when multiple computer processors are available to the user.
...
%		{reconcile}x assignments in alternate display trees are equal in subtree leaf set, only 
...
%		\item[lazyparstrat] Only internal---used for testing purposes.
		...
		\item[keep:[INT]] Sets the maximum number of graphs to be kept.
		The default is 10.
...
%		\item[reportNaiveData] Sets the data output and diagnosis.		
%		By default set to \texttt{True}, and diagnosis will report data as input.  Memory 
%		footprint for large data sets can be reduced by setting reportNaiveData to False. 
%		When False, diagnosis will be based on recoded characters that may be inscrutable.  
%		To report more legible diagnoses, graphs can be individually rediagnosed with 
%		reportNaiveData:True.  This should consume much less memory that the
%		graph search.
%		prealigned or qualitative 
...
%		\item{\texttt{set(compressResolutions:False)}\\Turns off softwired graph resolution 
%		compression.}
...
\label{subsec:support}
	...
	\begin{phygdescription}
		...
		~\cite{Felsenstein1985}) and calculations of optimality-based measures (Goodman
		-Bremer support ~\cite{Goodmanetal1982, Bremer1988}). The calculation of support 
		measures based on resampling is done at the fragment or ``block'' rather than at the 
		nucleotide level. For this reason, these data need to be collated into a block with the 
		command \texttt{reblock} (see Section \ref{subsec:reblock}). Goodman-Bremer 
		calculations are not affected by this behavior. 
...
%		Only internal---used for testing purposes.
		...
		\begin{description}
			...
			as optionally specified (TBR as default) to determine an upper bound on the 
			     NP-hard values (this is the method used in POY v2; \citealp{POY2} 
			     \textit{et seq.}).
		...
		or hardwired, \texttt{netadd}, \texttt{maxnetedges:5}, \texttt{atrandom} are added. 
		   ...
		   calculation of support values. By default (\texttt{FALSE}), the amount of 
		   parallelization is reduced. When set to \texttt{TRUE}, parallelization is maximized 
		   at the cost of memory. This reduces the memory footprint of the analysis.
