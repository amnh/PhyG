...
%	\end{center}
	...
	\subsection{Read}
		...
		Options prepended to the filename with a colon (``:'') modify the processing of the input file.
		\subsubsection{Arguments}
		    ...
		    by replacing tghe dashes that are to be treated as missing data with question marks (`?')
...
%		\subsubsection{Examples}
	...
	\subsection{Report}
		...
		\subsubsection{Description}
			Outputs various information to file or stderr.
		\subsubsection{Arguments}
				
		\subsubsection{Defaults}
				\texttt{report()} prints input data and output graph diagnosis  to stderr.
		\subsubsection{Examples}
	...
	\subsection{Run}
		...
		\subsubsection{Examples}
				...
				\item{\texttt{run("searchCommands.pg"} Executes "searchCommands.pg", which may contain commands defineing a common search strategy (e.g. \texttt{build}).}
			\end{itemize}
