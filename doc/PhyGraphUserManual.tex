...
%	\end{center}
	...
	\subsection{Build}
		...
		\noindent \texttt{character} Performs random addition sequence Wagner \citep{Farris1970} builds ($O(n^2)$) for tree construction.  If the graphtype is ``softwired'',
...
%		
	\subsection{Swap} 
		...
		\subsubsection{Description}
		Performs branch-swapping rearrangement on graphs.  These include ``SPR''  \citep{Dayhoff1969} and ``TBR''  \citep{Farris1988, swofford1990a}  refinement.
		
		\subsubsection{Arguments}
		\noindent \texttt{keep:n} Specifies that up to $n$ equally costly graphs are retained.
		
		\smallskip		
		\noindent \texttt{spr} Specifies that SPR refinement \citep{Dayhoff1969} is performed.
		...
		\smallskip		
		\noindent \texttt{tbr} Specifies that TBR refinement \citep{Farris1988, swofford1990a} is performed.
