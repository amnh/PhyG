...
%	\end{center}
	...
	%PhyGraph needs its own description rather than just being the successor of POY
	
	\section{Overview of program use}
	
	At present, \texttt{PhyG} is operated solely via command-line interface. %and invoked in a script.
...
%	\subsection{Command Reference}
	...
	\subsection{Read}
		...
		\subsubsection{Arguments}
			...
			\smallskip		
			\noindent \texttt{prefasta:} Specifies that the sequences are prealigned in a fasta format, leaving gap characters
			(``-'') in the sequences and alignment correspondences are not re-examined. This option exists to ensure proper parsing (and in case auto-format detection is incorrect).
			
			\smallskip		
			\noindent \texttt{prefastc:} Specifies that the sequences are prealigned in a fastc format, leaving gap characters
			(``-'') in the sequences and alignment correspondences are not re-examined. This option exists to ensure proper parsing (and in case auto-format detection is incorrect).
...
%		
	...
	%\bibliography{/Users/Ward/DropboxAMNH/Work_stuff/manus/big-refs-3.bib}
