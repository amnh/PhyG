...
%		\subsubsection{Examples}
	...
	\subsection{Report}
		...
		\subsubsection{Arguments}
				...
				\smallskip
				\noindent \texttt{tnt} Outputs data for each current graph as a file suitable for input to TNT \citep{Goloboffetal2008}.  Non-prealigned sequence characters are output as implied alignments.  If the current graph is a soft-wired network, the implied alignments are based on the display trees for each data block. TNT output is not available (at least currently) for hard-wired networks.  This graph type can be, however, converted to a soft-wired network and TNT output generated from that graph.
