...
\section{\phyg Command Structure}
	...
	\subsection{Brief description}
		...
		\phyg recognizes three types of command arguments: primitive values, label
		(e.g. 3), a real number or float (e.g. 2.0), a string (e.g. ``Chel.fas'') or a boolean 
		(True|False). Labeled arguments consist of an argument and an identifier 
		...
		ascribed an integer and a float, respectively.
		
		\begin{quote}
		set(graphfactor:nopenalty) 
		\end{quote}
		
		In this third and final example, the command \texttt{set} is followed by the 
