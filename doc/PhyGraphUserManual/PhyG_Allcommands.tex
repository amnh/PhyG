...
\subsection{Set}
	...
	\subsubsection{Arguments}
		\begin{description}
			...
			\texttt{transform(compressResolutions:True)} or \texttt{transform(softwiredMethod:Naive)} 
			...
			The W15 root cost is based on the same rationale as the parsimony network penalty of
			 ...
			 \item[softwiredMethod:Naive|ResolutionCache] Sets the algorithm for softwired graphs 
			 to the ``naive'' method of diagnosing all display trees as in \cite{Wheeler2015} or
...
\subsection{Swap} 
	...
	\subsubsection{Arguments}
		\begin{description}
			...
			\item[softwiredMethod:Naive|ResolutionCache] Specifies that the algorithm for softwired graphs 
			is set to the ``naive'' method of diagnosing all display trees as in \cite{Wheeler2015} or
			the ``Resolution Cache'' method of \cite{WheelerandWashburn2023}. This is an experimental 
			option that only operates on input graphs.
