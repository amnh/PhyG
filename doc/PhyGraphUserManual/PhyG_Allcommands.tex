...
\subsection{Build}
	...
	\begin{phygdescription}
		...
		specified within the command for distance builds. Refinement for \hl{character-based 
	...
	\begin{description}
		...
		\begin{description}
			...
			\texttt{atRandom} \hl{or first?} options.
		...
		as character trees and returned for further analysis \hl{do we need to explain why?}. Associated arguments of \texttt{distance} 
		include:
...
%		$n$ edges aware from their initial positions).
		...
		of graphs in both directions--roughly doubling time and memory footprint.
...
\label{subsec:read}
	...
	\begin{description}
		...
		\item [block:STRING] Specifies that the string contains block %\hl{ref?} 
...
%		specified \href{https://www.github.com/wardwheeler/euncon}{here} \citep{Wheeler2022}.
		...
		\begin{tcolorbox}[enhanced,fit to height=3.5cm,
  		colback=yellow!10!black!2!white,colframe=yellow!90!black,title=Note,
  		drop fuzzy shadow]
  		Sequences can be divided into smaller fragments using an assigned character. This 
		character is chosen by the user (unlike in POY, where the pound sign (`\textbf{\#}') was 
		used to partition datasets). Each fragment is treated as an individual character. When 
...
\label{subsec:search}
	...
	\subsubsection{Defaults}
		\texttt{search(instances:1, sections:30, \hl{keep:10})} Under the default parameters, \phyg will 
		perform 1 instance of a search for at most 30 seconds, keeping up to 10 graphs}.
