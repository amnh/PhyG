...
\subsection{Build}
	...
	\begin{description}
		...
		\begin{description}
			...
			display trees as specified by the integer in \texttt{displayTrees[:INTEGER]}, where 
			...
			\item[displayTrees[:INTEGER]] When the option \texttt{block} is specified, this variable 
		...
		\begin{description}
			\item[best:INTEGER] Applies only to \texttt{rdWag}. Specifies the number of trees retained 
		...
		\item [replicates:INTEGER] Applies to \texttt{rdWag} and \texttt{character}. Specifies the 
		number (INTEGER) of random addition sequences performed.
		
		\item[return:INTEGER] Applies to \texttt{rdWag} and \texttt{character}. This limits the 
			number of Wagner trees returned for further analysis. By default all graphs that 
			are built are returned, unless limited by \texttt{best:INTEGER} in distance analysis. 
...
%		$n$ edges aware from their initial positions).
		...
		\item [keep:INTEGER] Limits the number of returned graphs to the integer value specified. 
		...
		\item [pairs:INTEGER] Limits the number of graphs to be fused to the number of pairs as 
		specified by the INTEGER (as oppose to $\binom{m}{2}$ for $m$ graphs).
		...
		\item [spr[:INTEGER]] Causes the exchanged subgraphs to be tried at multiple positions (up to 
		...
		\item [tbr[:INTEGER]] Causes the exchanged subgraphs to be tried at multiple positions (up to 
...
\label{subsec:read}
	...
	\begin{phygdescription}
		...
		same input line of the command script, but only of the same data type. Prepending the file 
		type prevents any ambiguity when the file is parsed. If the data type is not specified, 
		it is important to verify that the data was interpreted properly (using the command 
		\texttt{report("STRING", data)} or by checking the output display in the \textit{Terminal} 
		window). \texttt{read} \texttt{read} can also use	wildcard expressions (`*' and `?'), 
		which can be useful when reading in multiple files of the same type. For example, 
		read(nucleotide:"*.fas*") imports all files of the FASTA format in the current directory 
		(in this case this will include files that end in both .fas and .fasta). Reading in FASTA 
		files of different data types, e.g. amino acid and nucleotide, will result in an error.
		Terminal names should not have spaces in the imported data file, otherwise the 
		names can be incorrectly interpreted by the program. } 
...
\item [tcm:] This refers to a file containing a custom-alphabet matrix that specifies varying 
	...
	\begin{example}
		\item{read(nucleotide:"~/Desktop/phyg/metazoa.fas", tcm:"sg1t4.mat")}\\ Reads the
