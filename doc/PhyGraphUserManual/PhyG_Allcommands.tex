...
\subsection{Search}
	...
	\begin{phygdescription}
		...
		Algorithm. The parameters and \hl{order} for this search are randomized. The arguments 
		specify the number of rounds of search (\texttt{instances}, default 1), and duration 
		...
		longer that the specified duration. \hl{how is this executed in parallel?} 
	...
	\begin{description}
		...
		\item[instances:n] \hl{Performs $n$ successive rounds of  search? How does instances
		differ from iterations?}
	...
	\subsubsection{Defaults}
		\texttt{search()} Performs 1 iteration of 30 seconds keeping up to 10 graphs \hl{30 seconds?}.
...
\subsection{Select}
	...
	\subsubsection{Arguments}
		\begin{description}
			...
			\item[best] Selects and keeps the graphs with the  best optimality value.
			
			\item[random] Keeps graphs chosen at random. \hl{should this be atRandom?}
			
			\item[arg:n] Keeps up to \texttt{n} graphs determined by the argument \hl{what does this mean?} 
			\hl{should this be unique?}.
		\end{description}
...
\subsection{Set}
	...
	\begin{phygdescription}
		...
		global settings during a run (see \hl{link to Transform}}
	\end{phygdescription}
			
	\subsubsection{Arguments}
		\begin{description}
			\item[compressResolutions:True|False] \hl{change to BOOL} Determines whether soft-wired graph 
			...
			\item[modelcomplexoty] \hl{details}
			
			\item[outgroup:STRING\hl{or LIDENT?}] Specifies the terminal to root the output trees. 
			This name must appear in quotes. 
			
			\item[partitioncharacter] Sets the character that is used to partition data \hl{what does this mean?}.
	...
	\subsubsection{Defaults} 
		There are no default \texttt{set} arguments. \hl{why say there are no defaults when all these defaults
		follow?} The default outgroup is the taxon whose name is 
...
\subsection{Support}
	...
	\begin{phygdescription}
		...
		are the resampling methods that are implemented. \hl{why mention Goodman-Bremer 
		here if it's not supported? Should we just comment this out until such time that it is implemented?}.}
	\end{phygdescription}
	...
	\subsubsection{Arguments}
		\begin{description}
			...
			\item[bootstrap] Calculates Bootstrap support. \hl{does the user have to specify the 
...
\subsection{Swap} 
	...
	\begin{phygdescription}
		...
		and ``TBR'' \citep{Farris1988, swofford1990a} refinement. \texttt[swap} is also used 
		as an argument for \texttt{build} to specify \hl{XXX} (see Section include link here to build).
	\end{phygdescription}
