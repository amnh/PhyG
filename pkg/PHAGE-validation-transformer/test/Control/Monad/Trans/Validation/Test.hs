...
testSuite :: TestTree
testSuite = testGroup "Validation Monad Transformer" [evaluationTLaws]


evaluationTLaws :: TestTree
evaluationTLaws = testGroup
    "ValidationT"
...
functorLaws :: forall f . (Arbitrary (f W), Eq (f W), Functor f, Show (f W)) => TestTree
functorLaws = testGroup
    "Functor Laws"
...
applicativeLaws
    :: forall f
     . (Applicative f, Arbitrary (f W), Arbitrary (f (Fun W W)), Eq (f W), Show (f W), Show (f (Fun W W)))
    => TestTree
applicativeLaws = testGroup
    "Applicative Laws"
    ...
    where
        ...
        applicativeHomomorphism (apply -> f) x = (pure f <*> pure x) === (pure (f x) :: f W)

        applicativeInterchange :: f (Fun W W) -> W -> Property
        applicativeInterchange (fmap apply -> x) y = (x <*> pure y) === (pure ($ y) <*> x)

        specializedLeftEffect :: f W -> f W -> Property
        specializedLeftEffect u v = (u *> v) === ((id <$ u) <*> v)

        specializedRightEffect :: f W -> f W -> Property
        specializedRightEffect u v = (u <* v) === liftA2 const u v

        functorRelation :: Fun W W -> f W -> Property
        functorRelation (apply -> f) x = fmap f x === (pure f <*> x)


monadLaws :: forall m . (Arbitrary (m W), Eq (m W), Monad m, Show (m W)) => TestTree
monadLaws = testGroup
    "Monad Laws"
...
monadFailLaws :: forall m . (Arbitrary (m W), Eq (m W), MonadFail m, Show (m W)) => TestTree
monadFailLaws = testGroup
    "MonadFail Laws"
    [testLaw leftNullification "Left Nullification" "fail s >>= f === fail s"]
    where
        leftNullification :: Fun W (m W) -> String -> Property
        leftNullification (apply -> f) s = (fail s >>= f) === (fail s :: m W)


monadTransLaws
    :: forall t m
     . (Arbitrary (m W), Eq (t m W), Monad m, Monad (t m), MonadTrans t, Show (m W), Show (t m W))
    => TestTree
monadTransLaws = testGroup
    "MonadTrans Laws"
    [ testLaw liftedPure      "Lifted Pure"      "lift . pure === pure"
    , testLaw bindComposition "Bind Composition" "lift (x >>= f) === lift x >>= (lift . f)"
    ]
    where
        liftedPure :: W -> Property
        liftedPure x = (lift . pure) x === (pure x :: t m W)

        bindComposition :: m W -> Fun W (m W) -> Property
        bindComposition x (apply -> f) = lift (x >>= f) === ((lift x >>= (lift . f)) :: t m W)
...
monadZipLaws
    ...
    => TestTree
monadZipLaws = testGroup
    "MonadZip Laws"
    [ testLaw naturality "Naturality" "fmap (f *** g) (mzip ma mb) === mzip (fmap f ma) (fmap g mb)"
    , testLaw
        infoPreservation
    ...
    where
        ...
        naturality (apply -> f) (apply -> g) x y = fmap (f *** g) (mzip x y) === mzip (fmap f x) (fmap g y)

        infoPreservation :: m W -> m W -> Property
        infoPreservation x y = (void x =/= void y) .||. (munzip (mzip x y) === (x, y))


altLaws :: forall f . (Alt f, Arbitrary (f W), Eq (f W), Monad f, Show (f W)) => TestTree
altLaws = testGroup
    "Alt Laws"
...
--    , "Right Distributivity"   "(m <!> n) >>- f === (m >>- f) <!> (m >>- f)"
    ...
    where
        ...
        altAssociativity x y z = ((x <!> y) <!> z) === (x <!> (y <!> z))

        altLeftCatch :: W -> f W -> Property
        altLeftCatch x y = (pure x <!> y) === pure x

        altLeftDistributivity1 :: Fun W W -> f W -> f W -> Property
        altLeftDistributivity1 (apply -> f) x y = (f <$> (x <!> y)) === ((f <$> x) <!> (f <$> y))
...
applyLaws = testGroup
    ...
    where
        ...
        composition (fmap apply -> x) (fmap apply -> y) z = ((.) <$> x <.> y <.> z) === (x <.> (y <.> z))

        leftInterchange :: Fun W W -> f (Fun W W) -> f W -> Property
        leftInterchange (apply -> f) (fmap apply -> x) y = (x <.> (f <$> y)) === ((. f) <$> x <.> y)

        rightInterchange :: Fun W (f W) -> f (Fun W W) -> f W -> Property
        rightInterchange (apply -> f) (fmap apply -> x) y = (f <$> (x <.> y)) === ((f .) <$> x <.> y)

        leftNullification :: Fun W W -> Fun W W -> f W -> f W -> Property
        leftNullification (apply -> f) (apply -> g) m n = ((f <$> m) .> (g <$> n)) === (g <$> (m .> n))

        rightNullification :: Fun W W -> Fun W W -> f W -> f W -> Property
        rightNullification (apply -> f) (apply -> g) m n = ((f <$> m) <. (g <$> n)) === (f <$> (m <. n))
...
bindLaws = testGroup
    ...
    where
        ...
        defJoin x = join x === (>>- id) x

        defBind :: Fun W (m W) -> m W -> Property
        defBind (apply -> f) x = (x >>- f) === join (fmap f x)

        defApply :: m (Fun W W) -> m W -> Property
        defApply (fmap apply -> f) x = (f <.> x) === (f >>- (<$> x))

        associativity1 :: Fun W (m W) -> Fun W (m W) -> m W -> Property
        associativity1 (apply -> f) (apply -> g) x = ((x >>- f) >>- g) === (x >>- (\a -> f a >>- g))

        associativity2 :: m (m (m W)) -> Property
        associativity2 x = (join . join) x === (join . fmap join) x


equalityLaws :: forall a . (Arbitrary a, Eq a, Show a) => TestTree
equalityLaws = testGroup
    "Equality Laws"
...
normalFormDataLaws :: forall a . (Arbitrary a, NFData a, Show a) => TestTree
normalFormDataLaws = testGroup "NFData Laws" [testLaw finiteReduction "Finiteness" "rnf x =/= _|_"]
    where
        finiteReduction :: a -> Property
        finiteReduction x = rnf x === ()


orderingLaws :: forall a . (Arbitrary a, Ord a, Show a) => TestTree
orderingLaws = testGroup
    "Ordering Laws"
    [ testLaw symmetry      "Symmetry"      "x >= y ==> y <= x"
    ...
    where
        ...
        transitivity1 x y z = (x < y && y < z) ==> x < z

        transitivity2 :: a -> a -> a -> Property
        transitivity2 x y z = (x > y && y > z) ==> x > z


semigroupLaws :: forall a . (Arbitrary a, Eq a, Semigroup a, Show a) => TestTree
semigroupLaws = testGroup
    "Semigroup Laws"
...
showProperties :: forall a . (Arbitrary a, Show a) => TestTree
showProperties = testGroup
    ...
    where
        ...
        finiteString x = (rnf . show) x === ()

        nonNullString :: a -> Bool
        nonNullString = not . null . show


foldableLaws :: forall f . (Arbitrary (f W), Foldable f, Show (f W)) => TestTree
foldableLaws = testGroup
    "Foldable Laws"
    [ testLaw
        testFoldlFoldMap
        "Dual Endomorphism"
        "foldl' f z t === appEndo (getDual (foldMap (Dual . Endo . flip f) t)) z"
    , testLaw testFoldrFoldMap         "Fold-map Endomorphism"                 "foldr f z t === appEndo (foldMap (Endo . f) t ) z"
    , testLaw testFoldr                "Fold-right List Equivelency"           "foldr f z === foldr f z . toList"
    , testLaw testFoldl                "Fold-right List Equivelency"           "foldl' f z === foldl' f z . toList"
    , testLaw testFoldr1               "Non-empty Fold-right List Equivelency" "foldr1 f === foldr1 f . toList"
    , testLaw testFoldl1               "Non-empty Fold-left List Equivelency"  "foldl1 f === foldl1 f . toList"
    , testLaw testNull                 "Zero-length Nullability Implication"   "null === (0 ==) . length"
    , testLaw testLength               "Length List Equivelency"               "length === length . toList"
    , testLaw testInclusionConsistency "Inclusion Consistency"                 "elem e =/= notElem e"
    , testLaw testMax                  "Max Fold-map Equivelency"              "maximum === getMax . foldMap Max"
    , testLaw testMin                  "Min Fold-map Equivelency"              "minimum === getMin . foldMap Min"
    , testLaw testSum                  "Sum Fold-map Equivelency"              "sum === getSum . foldMap Sum"
    , testLaw testProduct              "Product Fold-map Equivelency"          "product === getProduct . foldMap Product"
    , testLaw testFirst                "First Fold-map Equivelency"            "head . toList === getFirst . foldMap First"
    , testLaw testLast                 "Last Fold-map Equivelency"             "last . toList === getLast . foldMap Last"
    , testLaw testAll                  "All Fold-map Equivelency"              "all f === getAll . foldMap (All . f)"
    , testLaw testAny                  "Any Fold-map Equivelency"              "any f === getAny . foldMap (Any . f)"
    ]
    where
        testFoldrFoldMap :: Fun (W, W) W -> W -> f W -> Property
        testFoldrFoldMap (applyFun2 -> f) z x = foldr f z x === appEndo (foldMap (Endo . f) x) z
        ...
        testFoldr (applyFun2 -> f) z x = foldr f z x === (foldr f z . toList) x

        testFoldl :: Fun (W, W) W -> W -> f W -> Property
        testFoldl (applyFun2 -> f) z x = foldl' f z x === (foldl' f z . toList) x

        testFoldr1 :: Fun (W, W) W -> f W -> Property
        testFoldr1 (applyFun2 -> f) x = (not . null) x ==> foldr1 f x === (foldr1 f . toList) x

        testFoldl1 :: Fun (W, W) W -> f W -> Property
        testFoldl1 (applyFun2 -> f) x = (not . null) x ==> foldl1 f x === (foldl1 f . toList) x

        testNull :: f W -> Property
        testNull x = null x === ((0 ==) . length) x

        testLength :: f W -> Property
        testLength x = length x === (length . toList) x

        testInclusionConsistency :: (W, f W) -> Property
        testInclusionConsistency (e, x) = elem e x =/= notElem e x

        testMax :: f W -> Property
        testMax x = (not . null) x ==> maximum x === (getMax . foldMap Max) x

        testMin :: f W -> Property
        testMin x = (not . null) x ==> minimum x === (getMin . foldMap Min) x

        testSum :: f W -> Property
        testSum x = sum x === (getSum . foldMap Sum) x

        testProduct :: f W -> Property
        testProduct x = product x === (getProduct . foldMap Product) x

        testFirst :: f W -> Property
        testFirst x =
            (not . null) x ==> (Just . head . toList) x === (fmap getFirst . foldMap (Just . First)) x

        testLast :: f W -> Property
        testLast x = (not . null) x ==> (Just . last . toList) x === (fmap getLast . foldMap (Just . Last)) x

        testAll :: Fun W Bool -> f W -> Property
        testAll (apply -> f) x = all f x === (getAll . foldMap (All . f)) x

        testAny :: Fun W Bool -> f W -> Property
        testAny (apply -> f) x = any f x === (getAny . foldMap (Any . f)) x


traversableLaws
    :: forall f
     . (Arbitrary (f W), Eq (f Bool), Eq (f W), Traversable f, Show (f W), Show (f Bool))
    => TestTree
traversableLaws = testGroup
    "Traversable Laws"
    ...
    where
        ...
        equality (apply -> f) x = traverse f x === mapM f x
