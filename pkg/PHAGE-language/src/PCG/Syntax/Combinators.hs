...
import Control.Monad.Free qualified as F
...
import PCG.Syntax.Primitive qualified as P
...
choiceFrom opts = case toList opts of
...
argIds strs arg = case toList strs of
...
parseCommand
    :: (FoldCase (Tokens s), MonadParsec e s m, Token s ~ Char, VisualStream s)
    => CommandSpecification a
    -> m a
...
runSyntax
    :: (FoldCase (Tokens s), MonadParsec e s m, VisualStream s, Token s ~ Char)
    => Ap SyntacticArgument a
    -> m a
runSyntax = intercalateEffect comma . runAp' noEffect apRunner where noEffect = toPermutation voidEffect
...
apRunner effect (ExactlyOneOf ps) =
    toPermutation $ runPermutation effect *> choice (runPermutation . runAp (apRunner voidEffect) <$> ps)
apRunner effect (ArgumentList p) =
    toPermutation $ runPermutation effect *> runPermutation (parseArgumentList p)
apRunner effect (DefaultValue p v) = toPermutationWithDefault v $ runPermutation effect *> runPermutation
    (runAp (apRunner (toPermutation voidEffect)) p)
...
parseArgumentList argListVal = toPermutation $ begin *> datum <* close
    where
        bookend :: forall e s (f :: Type -> Type) a . (Token s ~ Char, MonadParsec e s f) => f a -> f ()
...
comma = try (whitespace *> separator *> whitespace)
    where separator = char ',' <?> "',' separating arguments"
