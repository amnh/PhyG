...
bool :: MonadFree PrimitiveValue m => m Bool
bool = liftF $ PBool id


-- |
-- A integral value embedded in a Free computational context
int :: MonadFree PrimitiveValue m => m Int
int = liftF $ PInt id


-- |
-- A real value embedded in a Free computational context
real :: MonadFree PrimitiveValue m => m Double
real = liftF $ PReal id
...
time :: MonadFree PrimitiveValue m => m DiffTime
time = liftF $ PTime id


-- |
-- A literal value embedded in a Free computational context
value :: MonadFree PrimitiveValue m => String -> m ()
...
parsePrimitive
    :: (FoldCase (Tokens s), MonadParsec e s m, VisualStream s, Token s ~ Char) => PrimitiveValue (m a) -> m a
...
parsePrimitive (PValue str x) = valueParser >>= x
    where valueParser = typeMismatchContext (valueValue str) (TypeOfValue str)
...
numValue = hidden signedNum <?> "number" where signedNum = signed whitespace scientific


realValue :: (MonadParsec e s m, Token s ~ Char) => m Double
realValue = label (getPrimitiveName TypeOfReal) $ either id id . toBoundedRealFloat <$> numValue


textValue :: forall e s m . (MonadParsec e s m, VisualStream s, Token s ~ Char) => m ShortText -- (Tokens s)
            ...
            -- literal character value
                ...
                characterEscaping (TrivialError pos uxpItems expItems) = TrivialError pos uxpItems' expItems'
                    where
                        ...
                        f :: forall t . ErrorItem Char -> ErrorItem t
                        f EndOfInput = EndOfInput
                        f (Tokens ts) =
                            Label
                                .  NE.fromList
                                $  "invalid escape sequence character: "
                                <> showTokens (Proxy :: Proxy s) ts
...
timeValue = do
    ...
    _       <- char ':'
    minutes <- decimal
    let totalSeconds = days * 60 * 60 * 24 + hours * 60 * 60 + minutes * 60
...
typeMismatchContext p targetType = do
    ...
    case parsedPrimitive of
        ...
        Just (str, parseResult) ->
            ...
            in  if targetType == resultType || targetType == TypeOfReal && resultType == TypeOfInt
                    then p
                    else
                        let uxpMsg = Just . Label . NE.fromList $ fold
                                [ getPrimitiveName parseResult
                                , " '"
                                , chunkToTokens (Proxy :: Proxy s) str
                                , "'"
                                ]
