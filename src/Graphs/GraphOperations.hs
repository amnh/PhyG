...
getDecoratedDisplayTreeList inCanonicalGraph (inDisplayL, blockIndex) =
    if LG.isEmpty inCanonicalGraph
        then error "Empty canonical graph in getDecoratedDisplayTree"
        else -- get edges and nodes for canonical graph
...
makeDisplayNodes canonicalNodeList blockIndex inDisplayNodeList =
    if length canonicalNodeList /= length inDisplayNodeList
        then error "Canonical and display node lists are unequal in length"
        else fmap (extractBlockNodeInfo blockIndex) canonicalNodeList
