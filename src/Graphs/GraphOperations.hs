...
ladderizeGraph' inGraph nodeList
    | LG.isEmpty inGraph = LG.empty
    | otherwise = case nodeList of
        [] → inGraph
        firstNode : otherNodes →
            let -- these are roots, network, tree, leaf nodes
                okNodeDegrees = [(0, 2), (1, 2), (2, 1), (1, 0)]
                inOuts@(inEdgeList, outEdgeList) = LG.getInOutEdges inGraph firstNode
                inOutPairLength = (length inEdgeList, length outEdgeList)
            in  case inOutPairLength `elem` okNodeDegrees of
                    -- node ok to keep
                    True → ladderizeGraph' inGraph otherNodes
                    -- node edges need modification
                    False →
                        let newGraph = resolveNode inGraph firstNode inOuts inOutPairLength
                        in  ladderizeGraph' newGraph (LG.nodes newGraph)
...
selectGraphs selectType numberToKeep threshold inGraphList =
    let convertReduced2GenPhyloGraph ∷ (Monoid e) ⇒ (a, b, c, d, f) → (a, b, c, d, e, f)
        convertReduced2GenPhyloGraph (a, b, c, d, f) = (a, b, c, d, mempty, f)

        convertGenPhyloGraph2Reduced ∷ (a, b, c, d, e1, e2) → (a, b, c, d, e2)
...
selectPhylogeneticGraph
    ...
    → [GenPhyloGraph a b]
    → PhyG [GenPhyloGraph a b]
selectPhylogeneticGraph _ _ [] = pure []
...
createUniqueBoolList nonZeroEdges = \case
    [] → reverse . fmap snd
    firstGraph : otherGraphs → \case
        [] → createUniqueBoolList nonZeroEdges otherGraphs [(firstGraph, True)]
        boolAccum →
            let value = null . filter id $ fmap (topologicalEqual nonZeroEdges firstGraph . fst) boolAccum
            in  createUniqueBoolList nonZeroEdges otherGraphs $ (firstGraph, value) : boolAccum
...
getEdgeMinLengthToNode edgeList (node, _) = case L.find ((== node) . snd3) edgeList of
    -- root node will be nor be in in edge set and need so set > 0
    Nothing → 1 --  error ("Edge not found in getEdgeMinLengthToNode: node " <> (show node) <> " edge list " <> (show edgeList))
    Just (_, _, lens) → minLength lens
...
getBVUniqPhylogeneticGraph nonZeroEdges = \case
    -- inPhyloGraphList =
    [] → []
    inPhyloGraphList →
...
createBVUniqueBoolList = \case
    [] → reverse . fmap snd
    firstGraphList : rest → \case
        [] → createBVUniqueBoolList rest [(firstGraphList, True)]
        boolAccum →
            let value = null . filter id $ fmap ((== firstGraphList) . fst) boolAccum
            in  createBVUniqueBoolList rest $ (firstGraphList, value) : boolAccum
    ...
    | otherwise =
        let getProb ∷ Double → Double
            ...
            selectGraphByWeighting cmp v =
                let extractKey ∷ (a, Double, c, d, e, f) → Double
                    extractKey = getProb . (-) minCost . snd6
