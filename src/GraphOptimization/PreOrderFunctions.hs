...
preOrderTreeTraversal inGS finalMethod staticIA calculateBranchLengths hasNonExact rootIndex useMap (inSimple, inCost, inDecorated, blockDisplayV, blockCharacterDecoratedVV, inCharInfoVV) =
    let -- parallel setup
        ...
        updateLeafAction = updateLeafIABlockPar --updateLeafIABlock' zipWith sequential version
    ...
    in  if LG.isEmpty inDecorated
            then pure emptyPhylogeneticGraph -- error "Empty tree in preOrderTreeTraversal"
                ...
                if null blockCharacterDecoratedVV
                    then pure emptyPhylogeneticGraph
                        ...
                        {- for updateLeafIABlock'
                         $ blockPar doBlockAction (V.toList $ V.zip inCharInfoVV blockCharacterDecoratedVV) 
                        -}
                        
                        blockPar ← getParallelChunkTraverse
                        blockVal <- blockPar doBlockAction (V.toList $ V.zip inCharInfoVV blockCharacterDecoratedVV)
                        let preOrderBlockVect = V.fromList blockVal 
                        ...
                        softwiredUpdatedLeafIA ←
                            if graphType inGS /= SoftWired
                                then pure preOrderBlockVect
                                else
                                    ...
                                    in  do
                                            ...
                                            let blockCharDecNewLeafIA =
                                                    V.fromList $
                                                        blockIAPar (updateLeafAction maxLeafIndex) $
                                                            V.toList $
                        ...
                        if null blockCharacterDecoratedVV
                            then
                                error
                                    ( "Empty preOrderBlockVect in preOrderTreeTraversal at root index rootIndex: "
                                        <> show rootIndex
...
Sequentially via zipWith charcters within a block, called in parallel over blocks
...
updateIAFields charInfo origNode@(origIndex, origLabel) (_, newLabel) =
    ...
    in  if characterType `notElem` nonExactCharacterTypes
            then origNode
            else
                let updatedChar =
                        if characterType `elem` [SlimSeq, NucSeq]
                            then origChar{slimAlignment = slimAlignment newChar}
                            else
                                if characterType `elem` [WideSeq, AminoSeq]
                                    then origChar{wideAlignment = wideAlignment newChar}
                                    else
                                        if characterType == HugeSeq
                                            then origChar{hugeAlignment = hugeAlignment newChar}
    ...
    if False
        then inGraph
            ...
            let postOrderIATree = postOrderIAUnion inGraph charInfo [(rootIndex, fromJust $ LG.lab inGraph rootIndex)]
                preOrderIATree =
                    preOrderIA postOrderIATree rootIndex finalMethod charInfo $
                        zip [(rootIndex, fromJust $ LG.lab postOrderIATree rootIndex)] [(rootIndex, fromJust $ LG.lab postOrderIATree rootIndex)]
            in  -- trace ("MCIAU:" <> (U.getUnionFieldsNode $ vertData $ fromJust $ LG.lab postOrderIATree rootIndex) <> "\n" <> (U.getUnionFieldsNode $ vertData $ fromJust $ LG.lab postOrderIATree 0)
...
postOrderIAUnion inGraph charInfo inNodeList =
    if null inNodeList
        then inGraph
            ...
            in  -- checking sanity of data
                if V.null $ vertData nodeLabel
                    then error "Null vertData in postOrderIA"
                    else
                        if V.null $ V.head $ vertData nodeLabel
                            then -- missing data for taxon
                                error "Null vertData data in postOrderIA"
                            else -- leaf take assignment from alignment field

                                if nodeType' == LeafNode
                                    then -- set leaf union fields to preliminary or IA fields
                                        ...
                                        in  postOrderIAUnion newGraph charInfo (tail inNodeList)
                                    else -- HTU take create assignment from children

                                        let childNodes = LG.labDescendants inGraph inNode
                                            childTree = postOrderIAUnion inGraph charInfo childNodes
                                        in  -- trace ("Children: " <> (show  $ fmap fst childNodes)) (

                                            if length childNodes > 2
                                                then error ("Too many children in postOrderIA: " <> show (length childNodes))
                                                else -- in 1 out 1 vertex
                                                    ...
                                                    if length childNodes == 1
                                                        then
                                                            ...
                                                            in  -- sanity checks
                                                                if isNothing (LG.lab childTree (fst $ head childNodes))
                                                                    then error ("No label for node: " <> show (fst $ head childNodes))
                                                                    else
                                                                        if V.null $ vertData childLabel
                                                                            then error "Null vertData in postOrderIA"
                                                                            else
                                                                                if V.null $ V.head $ vertData childLabel
                                                                                    then error "Null head vertData data in postOrderIA"
                                                                                    else
                                                                                        ...
                                                                                        in  -- trace ("PostO1Child: " <> (show nodeIndex) <> " " <> (show $ slimFinal childCharacter))
                                                            ...
                                                            if length childNodes == 2
                                                                then
                                                                    let childIndices = fmap fst childNodes
                                                                        ...
                                                                        newLabel = nodeLabel{vertData = V.singleton (V.singleton newCharacter), nodeType = nodeType'}
                                                                        newGraph = LG.insEdges (inNodeEdges <> outNodeEdges) $ LG.insNode (nodeIndex, newLabel) $ LG.delNode nodeIndex childTree
                                                                    in  -- trace ("PostO2hildren: " <> (show nodeIndex) <> " " <> (show $ slimFinal newCharacter) <> " " <> (show $ nodeType newLabel)) -- <> " From: " <> (show childlabels))
                                                                        postOrderIAUnion newGraph charInfo (tail inNodeList)
...
-}
preOrderIA
    ∷ DecoratedGraph → Int → AssignmentMethod → CharInfo → [(LG.LNode VertexInfo, LG.LNode VertexInfo)] → DecoratedGraph
preOrderIA inGraph rootIndex finalMethod charInfo inNodePairList =
    if null inNodePairList
        then inGraph
        else
            ...
            in  -- trace ("PreIA Node:" <> (show nodeIndex) <> " " <> (show $ nodeType nodeLabel) <> " " <> (show (fmap fst $ fmap fst inNodePairList,fmap fst $ fmap snd inNodePairList))) (
                -- checking sanity of data
                if V.null $ vertData nodeLabel
                    then error "Null vertData in preOrderIA"
                    else
                        if V.null $ V.head $ vertData nodeLabel
                            then error "Null vertData data in preOrderIA"
                            else
                                if length childNodes > 2
                                    then error ("Too many children in preOrderIA: " <> show (length childNodes))
                                    else -- leaf done in post-order

                                        if nodeType nodeLabel == LeafNode
                                            then preOrderIA inGraph rootIndex finalMethod charInfo (tail inNodePairList)
                                            else
                                                if nodeType nodeLabel == RootNode || nodeIndex == rootIndex
                                                    then
                                                        let newCharacter
                                                                | characterType `elem` [SlimSeq, NucSeq] =
                                                                    inCharacter
                                                                        { slimIAFinal = extractMediansGapped $ slimIAPrelim inCharacter'
                                                                        , slimFinal = extractMedians $ slimGapped inCharacter'
                                                                        }
                                                                | characterType `elem` [WideSeq, AminoSeq] =
                                                                    inCharacter
                                                                        { wideIAFinal = extractMediansGapped $ wideIAPrelim inCharacter'
                                                                        , wideFinal = extractMedians $ wideGapped inCharacter'
                                                                        }
                                                                | characterType == HugeSeq =
                                                                    inCharacter
                                                                        { hugeIAFinal = extractMediansGapped $ hugeIAPrelim inCharacter'
                                                        ...
                                                        in  -- trace ("PreIARoot: " <> (show nodeIndex) <> " IAFinal: " <> (show $ slimIAFinal newCharacter) <> " Final: " <> (show $ slimFinal newCharacter))
                                                            preOrderIA newGraph rootIndex finalMethod charInfo (tail inNodePairList <> zip childNodes parentNodeList)
                                                    else -- single child, take parent final assignments, but keep postorder assignments
                                                        ...
                                                        if length childNodes == 1
                                                            then
                                                                let newCharacter
                                                                        | characterType `elem` [SlimSeq, NucSeq] =
                                                                            inCharacter
                                                                                { slimFinal = slimFinal parentCharacter
                                                                                , slimIAFinal = slimIAFinal parentCharacter
                                                                                }
                                                                        | characterType `elem` [WideSeq, AminoSeq] =
                                                                            inCharacter
                                                                                { wideFinal = wideFinal parentCharacter
                                                                                , wideIAFinal = wideIAFinal parentCharacter
                                                                                }
                                                                        | characterType == HugeSeq =
                                                                            inCharacter
                                                                                { hugeFinal = hugeFinal parentCharacter
                                                                    ...
                                                                    newGraph = LG.insEdges (inNodeEdges <> outNodeEdges) $ LG.insNode (nodeIndex, newLabel) $ LG.delNode nodeIndex inGraph
                                                                    parentNodeList = replicate (length childNodes) (nodeIndex, newLabel)
                                                                in  -- trace ("PreIANet: " <> (show nodeIndex) <> " IAFinal: " <> (show $ slimIAFinal newCharacter) <> " Final: " <> (show $ slimFinal newCharacter))
                                                                    preOrderIA newGraph rootIndex finalMethod charInfo (tail inNodePairList <> zip childNodes parentNodeList)
                                                            else -- 2 children, make 3-way
                                                                ...
                                                                let finalCharacter = M.makeIAFinalCharacter finalMethod charInfo inCharacter parentCharacter -- leftChar rightChar
                                                                    ...
                                                                    newGraph = LG.insEdges (inNodeEdges <> outNodeEdges) $ LG.insNode (nodeIndex, newLabel) $ LG.delNode nodeIndex inGraph
                                                                    parentNodeList = replicate (length childNodes) (nodeIndex, newLabel)
                                                                in  -- trace ("PreIATree: " <> (show nodeIndex) <> " IAFinal: " <> (show $ slimIAFinal finalCharacter) <> " Final: " <> (show $ slimFinal finalCharacter))
                                                                    preOrderIA newGraph rootIndex finalMethod charInfo (tail inNodePairList <> zip childNodes parentNodeList)
...
doCharacterTraversal inGS finalMethod staticIA rootIndex inCharInfo inGraph =
    ...
    in  -- remove these two lines if working
        ...
        in  -- hope this is the most efficient way since all nodes have been remade
    ...
    let nothingVertData = U.copyToNothing (vertData inNodeLabel)
        newVertData =
            createFinalAssignmentOverBlocks
                ...
                False
                False
    in  (inNodeIndex, inNodeLabel{vertData = newVertData})
    ...
    if null nodesToUpdate
        then updatedNodes
            ...
            let (firstNode, firstParent, isLeft) = head nodesToUpdate
                ...
                firstParentVertData =
                    if length firstParents == 1
                        then vertData $ snd firstParent
                        else vertData $ snd $ head firstParents

                secondParentData =
                    if length firstParents == 1
                        then U.copyToNothing firstParentVertData
                ...
                childrenTriple' =
                    if graphType inGS == HardWired
                        then filter (indeg2NotInNodeList inGraph (tail nodesToUpdate)) childrenTriple
                        else childrenTriple
            in  -- trace (U.prettyPrintVertexInfo $ snd newFirstNode)
                ...
                makeFinalAndChildren
                    ...
                    staticIA
                    inGraph
                    (tail nodesToUpdate <> childrenTriple')
                    (newFirstNode : updatedNodes)
                    inCharInfo
...
assignPreorderStatesAndEdges inGS finalMethd calculateBranchEdges rootIndex preOrderBlockTreeVV useMap inCharInfoVV inGraph =
    ...
    if LG.isEmpty inGraph
        then error "Empty graph in assignPreorderStatesAndEdges"
        else -- trace ("In assign") (
            ...
            let postOrderNodes = LG.labNodes inGraph
                ...
                newEdgeList =
                    if graphType inGS == Tree || graphType inGS == HardWired
                        then
                            if useMap
                                then fmap (updateEdgeInfoTreeMap finalMethd inCharInfoVV nodeMap) postOrderEdgeList
            ...
            in  -- make new graph
                ...
                if calculateBranchEdges
                    then LG.mkGraph newNodeList newEdgeList
...
updateNodeWithPreorder
    ∷ V.Vector (V.Vector DecoratedGraph) → V.Vector (V.Vector CharInfo) → LG.LNode VertexInfo → LG.LNode VertexInfo
updateNodeWithPreorder preOrderBlockTreeVV inCharInfoVV postOrderNode =
    ...
    in  (fst postOrderNode, nodeLabel{vertData = newNodeVertData})
...
updatePreorderCharacter nodeIndex preOrderTree postOrderCharacter charInfo =
    ...
    in  -- this can heppen in naked parent node of prunned subGraph in branch swapping
        if isNothing maybePreOrderNodeLabel
            then emptyCharacter
            else -- error ("Nothing node label in updatePreorderCharacter node: " <> show nodeIndex)
...
updateCharacter postOrderCharacter preOrderCharacter localCharType
    ...
    | localCharType == AlignedWide =
        postOrderCharacter
            { alignedWidePrelim = alignedWidePrelim preOrderCharacter
    ...
    | localCharType == AlignedHuge =
        postOrderCharacter
            { alignedHugePrelim = alignedHugePrelim preOrderCharacter
    ...
    | localCharType == SlimSeq || localCharType == NucSeq =
        postOrderCharacter
            { slimAlignment = slimAlignment preOrderCharacter
    ...
    | localCharType == WideSeq || localCharType == AminoSeq =
        postOrderCharacter
            { wideAlignment = wideAlignment preOrderCharacter
    ...
    | localCharType == HugeSeq =
        postOrderCharacter
            { hugeAlignment = hugeAlignment preOrderCharacter
...
updateEdgeInfoSoftWired finalMethod inCharInfoVV blockTreePairVV rootIndex (uNode, vNode, edgeLabel) =
    if V.null blockTreePairVV
        then error "Empty node-edge pair vector in updateEdgeInfoSoftWired"
        else
...
getEdgeCharacterWeightSoftWired finalMethod uNode vNode rootIndex inCharInfo (nodeVect, edgeVect) =
    ...
    in  -- if edge not present and not around root then return  no costs
        if isNothing foundVertexPair
            then (0, 0)
...
getEdgeVerts
    ∷ Int → Int → Int → V.Vector (LG.LNode VertexInfo) → V.Vector (LG.LEdge EdgeInfo) → Maybe (VertexInfo, VertexInfo)
getEdgeVerts uNode vNode rootIndex nodeVect edgeVect =
    ...
    if (uNode >= V.length nodeVect) || (vNode >= V.length nodeVect)
        then Nothing
        else
            ( if edgeInVect (uNode, vNode) edgeVect || (edgeInVect (rootIndex, uNode) edgeVect && edgeInVect (rootIndex, vNode) edgeVect)
                then Just (snd $ nodeVect V.! uNode, snd $ nodeVect V.! vNode)
                else Nothing
            )
...
edgeInVect (u, v) edgeVect =
    not (V.null edgeVect)
        && ( let (a, b, _) = V.head edgeVect
             in  ((((u, v) == (a, b)) || ((v, u) == (a, b))) || edgeInVect (u, v) (V.tail edgeVect))
           )
...
-}
updateEdgeInfoTree
    ∷ AssignmentMethod → V.Vector (V.Vector CharInfo) → V.Vector (LG.LNode VertexInfo) → LG.LEdge EdgeInfo → LG.LEdge EdgeInfo
updateEdgeInfoTree finalMethod inCharInfoVV nodeVector (uNode, vNode, edgeLabel) =
    if V.null nodeVector
        then error "Empty node list in updateEdgeInfo"
        else
...
-}
updateEdgeInfoTreeMap
    ∷ AssignmentMethod → V.Vector (V.Vector CharInfo) → MAP.Map Int (LG.LNode VertexInfo) → LG.LEdge EdgeInfo → LG.LEdge EdgeInfo
updateEdgeInfoTreeMap finalMethod inCharInfoVV nodeMap (uNode, vNode, edgeLabel) =
    if MAP.null nodeMap
        then error "Empty node MAP in updateEdgeInfo"
        else
...
-}
getEdgeWeight
    ∷ AssignmentMethod → V.Vector (V.Vector CharInfo) → V.Vector (LG.LNode VertexInfo) → (Int, Int) → (VertexCost, VertexCost)
getEdgeWeight finalMethod inCharInfoVV nodeVector (uNode, vNode) =
    if V.null nodeVector
        then error "Empty node list in getEdgeWeight"
        else -- trace ("GEW: " <> (show $ fmap fst nodeVector) <> " " <> (show (uNode, vNode))) (
...
-}
getEdgeWeightMap
    ∷ AssignmentMethod → V.Vector (V.Vector CharInfo) → MAP.Map Int (LG.LNode VertexInfo) → (Int, Int) → (VertexCost, VertexCost)
getEdgeWeightMap finalMethod inCharInfoVV nodeMap (uNode, vNode) =
    if MAP.null nodeMap
        then error "Empty node map in getEdgeWeight"
        else -- trace ("GEWM: " <> (show $ MAP.toList nodeMap) <> " " <> (show (uNode, vNode))) (
...
getBlockCostPairsFinal
    ∷ AssignmentMethod → V.Vector CharacterData → V.Vector CharacterData → V.Vector CharInfo → (VertexCost, VertexCost)
    ...
    in  -- no nded to do nochange/change--all recoded in that case
        if thisCharType == Add
            then
                ...
                in  (minCost, maxCost)
            else -- assumes noChangeCost < changeCost for PMDL/ML

                if thisCharType == NonAdd
                    then
                        ...
                        in  if lNoChangeCost == 0.0
                                then (minCost, maxCost)
                        ...
                        if thisCharType `elem` packedNonAddTypes
                            then
                                ...
                                if thisCharType == Matrix
                                    then
                                        let minMaxListList =
                                                V.zipWith
                                                    (minMaxMatrixDiff thisMatrix)
                                                    (fmap getLowestCostMatrixStates (matrixStatesFinal uCharacter))
                                                    (fmap getLowestCostMatrixStates (matrixStatesFinal vCharacter))
                                        ...
                                        if thisCharType `elem` prealignedCharacterTypes
                                            then
                                                let (minDiff, maxDiff) =
                                                        unzip $
                                                            zipWith
                                                                (M.generalSequenceDiff thisMatrix (length thisMatrix))
                                                                (GV.toList $ alignedSlimFinal uCharacter)
                                                                (GV.toList $ alignedSlimFinal vCharacter)
                                                    minCost = thisWeight * fromIntegral (sum minDiff)
                                                    maxCost = thisWeight * fromIntegral (sum maxDiff)
                                                in  (minCost, maxCost)
                                            else
                                                if thisCharType == SlimSeq || thisCharType == NucSeq
                                                    then
                                                        let minMaxDiffList =
                                                                if finalMethod == DirectOptimization
                                                                    then
                                                                        ...
                                                                        zipWith
                                                                            (M.generalSequenceDiff thisMatrix (length thisMatrix))
                                                                            (GV.toList $ slimIAFinal uCharacter)
                                                                            (GV.toList $ slimIAFinal vCharacter)

                                                            (minDiff, maxDiff) = unzip minMaxDiffList
                                                            minCost = thisWeight * fromIntegral (sum minDiff)
                                                            maxCost = thisWeight * fromIntegral (sum maxDiff)
                                                        in  -- trace ("MMDL: " <> (show minCost) <> " " <> (show maxCost))
                                                            (minCost, maxCost)
                                                    else
                                                        if thisCharType == WideSeq || thisCharType == AminoSeq
                                                            then
                                                                let minMaxDiffList =
                                                                        if finalMethod == DirectOptimization
                                                                            then
...
maxMinIntervalDiff (a, b) (x, y) =
    ...
    in  (min upper lower, max upper lower)
...
createFinalAssignmentOverBlocks inGS finalMethod staticIA childType childBlockData parentBlockData parent2BlockDataM charInfo isLeft isInOutDegree1 isIn2Out1 =
    ...
    V.zipWith3
        (assignFinal inGS finalMethod staticIA childType isLeft charInfo isInOutDegree1 isIn2Out1)
        childBlockData
        parentBlockData
        parent2BlockDataM
...
setFinal inGS finalMethod staticIA childType isLeft charInfo isIn1Out1 isIn2Out1 childChar parentChar parent2CharM =
    ...
    in  -- Three cases, Root, leaf, HTU
        -- trace ("set final:" <> (show (finalMethod, staticIA)) <> " " <> (show childType) <> " " <> (show isLeft) <> " " <> (show isIn1Out1) <> " " <> (show isIn2Out1)) (
        if childType == RootNode
            then
                if localCharType == Add
                    then childChar{rangeFinal = snd3 $ rangePrelim childChar}
                    else
                        if localCharType == NonAdd
                            then childChar{stateBVFinal = snd3 $ stateBVPrelim childChar}
                            else
                                if localCharType `elem` packedNonAddTypes
                                    then childChar{packedNonAddFinal = snd3 $ packedNonAddPrelim childChar}
                                    else
                                        if localCharType == Matrix
                                            then childChar{matrixStatesFinal = setMinCostStatesMatrix (fromEnum symbolCount) (matrixStatesPrelim childChar)}
                                            else
                                                if localCharType == AlignedSlim
                                                    then childChar{alignedSlimFinal = extractMedians $ alignedSlimPrelim childChar}
                                                    else
                                                        if localCharType == AlignedWide
                                                            then childChar{alignedWideFinal = extractMedians $ alignedWidePrelim childChar}
                                                            else
                                                                if localCharType == AlignedHuge
                                                                    then childChar{alignedHugeFinal = extractMedians $ alignedHugePrelim childChar}
                                                                    else -- need to set both final and alignment for sequence characters

                                                                        if (localCharType == SlimSeq) || (localCharType == NucSeq)
                                                                            then
                                                                                let finalAssignment' = extractMedians $ slimGapped childChar
                                                                                in  -- trace ("TNFinal-Root: " <> (show finalAssignment') <> " " <> (show (GV.length finalAssignment', slimGapped childChar))) $
                                                                                    -- traceNoLF ("TNFinal-Root") $
                                                                                    if staticIA
                                                                                        then childChar{slimIAFinal = extractMediansGapped $ slimIAPrelim childChar}
                                                                                        else
                                                                                            childChar
                                                                                                { slimFinal = finalAssignment'
                                                                                                , slimAlignment -- slimGapped childChar
                                                                                                  =
                                                                                                    if isTree
                                                                                                        then slimGapped childChar
                                                                                                        else mempty
                                                                                                }
                                                                            else
                                                                                if (localCharType == WideSeq) || (localCharType == AminoSeq)
                                                                                    then
                                                                                        ...
                                                                                        in  if staticIA
                                                                                                then childChar{wideIAFinal = extractMediansGapped $ wideIAPrelim childChar}
                                                                                                else
                                                                                                    childChar
                                                                                                        { wideFinal = finalAssignment'
                                                                                                        , wideAlignment -- wideGapped childChar
                                                                                                          =
                                                                                                            if isTree
                                                                                                                then wideGapped childChar
                                                                                                                else mempty
                                                                                                        }
                                                                                    else
                                                                                        if localCharType == HugeSeq
                                                                                            then
                                                                                                ...
                                                                                                in  if staticIA
                                                                                                        then childChar{hugeIAFinal = extractMediansGapped $ hugeIAPrelim childChar}
                                                                                                        else
                                                                                                            childChar
                                                                                                                { hugeFinal = finalAssignment'
                                                                                                                , hugeAlignment -- hugeGapped childChar
                                                                                                                  =
                                                                                                                    if isTree
                                                                                                                        then hugeGapped childChar
                                                                                                                        else mempty
                                                                                                                }
                                                                                            else error ("Unrecognized/implemented character type: " <> show localCharType)
            else
                if childType == LeafNode
                    then -- since leaf no neeed to precess final alignment fields for sequence characters
                                            ...
                                            then childChar{packedNonAddFinal = snd3 $ packedNonAddPrelim childChar}
                                                ...
                                                if localCharType == Matrix
                                                    then childChar{matrixStatesFinal = setMinCostStatesMatrix (fromEnum symbolCount) (matrixStatesPrelim childChar)}
                                                        ...
                                                        if localCharType == AlignedSlim
                                                            then childChar{alignedSlimFinal = extractMediansGapped $ alignedSlimPrelim childChar}
                                                            else
                                                                if localCharType == AlignedWide
                                                                    then childChar{alignedWideFinal = extractMediansGapped $ alignedWidePrelim childChar}
                                                                    else
                                                                        if localCharType == AlignedHuge
                                                                            then childChar{alignedHugeFinal = extractMediansGapped $ alignedHugePrelim childChar}
                                                                            else -- need to set both final and alignment for sequence characters

                                                                                if (localCharType == SlimSeq) || (localCharType == NucSeq)
                                                                                    then
                                                                                        let finalAlignment = doPreOrderWithParentCheck isLeft (slimAlignment parentChar) (slimGapped parentChar) (slimGapped childChar)
                                                                                        in  -- traceNoLF ("TNFinal-Leaf " <> (show isSingleParentIn1Out1) <> " ") $
                                                                                            if staticIA
                                                                                                then childChar{slimIAFinal = extractMediansGapped $ slimIAPrelim childChar}
                                                                                                else
                                                                                                    childChar
                                                                                                        { slimFinal = extractMedians $ slimGapped childChar -- finalAssignment'
                                                                                                        ...
                                                                                                        , slimIAFinal -- extractMediansGapped finalAlignment
                                                                                                          =
                                                                                                            if isTree
                                                                                                                then extractMediansGapped $ finalAlignment
                                                                                                                else mempty
                                                                                                        }
                                                                                    else -- )
                                                                                            ...
                                                                                            then
                                                                                                ...
                                                                                                in  if staticIA
                                                                                                        then childChar{wideIAFinal = extractMediansGapped $ wideIAPrelim childChar}
                                                                                                            ...
                                                                                                            childChar
                                                                                                                { wideFinal = extractMedians $ wideGapped childChar -- finalAssignment'
                                                                                                ...
                                                                                                if localCharType == HugeSeq
                                                                                                    then
                                                                                                        ...
                                                                                                        in  -- finalAssignment' = extractMedians finalAlignment

                                                                                                            if staticIA
                                                                                                                then childChar{hugeIAFinal = extractMediansGapped $ hugeIAPrelim childChar}
                                                                                                                else
                                                                                                                    childChar
                                                                                                                        { hugeFinal = extractMedians $ hugeGapped childChar -- finalAssignment'
                        ...
                        if childType == TreeNode && not isIn1Out1
                                                                ...
                                                                if localCharType == AlignedSlim
                                                                    then
                                                                        let alignedFinal =
                                                                                M.getFinal3WaySlim
                                                                                    (slimTCM charInfo)
                                                                                    (alignedSlimFinal parentChar)
                                                                                    (fst3 $ alignedSlimPrelim childChar)
                                                                                    (thd3 $ alignedSlimPrelim childChar)
                                                                        in  childChar{alignedSlimFinal = alignedFinal}
                                                                    else
                                                                        if localCharType == AlignedWide
                                                                            then
                                                                                let alignedFinal =
                                                                                        M.getFinal3WayWideHuge
                                                                                            (wideTCM charInfo)
                                                                                            (alignedWideFinal parentChar)
                                                                                            (fst3 $ alignedWidePrelim childChar)
                                                                                            (thd3 $ alignedWidePrelim childChar)
                                                                                in  childChar{alignedWideFinal = alignedFinal}
                                                                            else
                                                                                if localCharType == AlignedHuge
                                                                                    then
                                                                                        let alignedFinal =
                                                                                                M.getFinal3WayWideHuge
                                                                                                    (hugeTCM charInfo)
                                                                                                    (alignedHugeFinal parentChar)
                                                                                                    (fst3 $ alignedHugePrelim childChar)
                                                                                                    (thd3 $ alignedHugePrelim childChar)
                                                                                        in  childChar{alignedHugeFinal = alignedFinal}
                                                                                    else -- need to set both final and alignment for sequence characters

                                                                                        if (localCharType == SlimSeq) || (localCharType == NucSeq)
                                                                                            then -- traceNoLF ("TNFinal-Tree " <> (show isSingleParentIn1Out1) <> " ") $
                                                                                                    ...
                                                                                                    finalAssignmentDO =
                                                                                                        if finalMethod == DirectOptimization
                                                                                                            then
                                                                                                                ...
                                                                                                                in  extractMedians finalAssignmentDONonGapped
                                                                                                            else -- really could/should be mempty since overwritten by IA later
                                                                                                                removeGapAndNil $ extractMedians finalGapped
                                                                                                in  -- trace ("TNFinal-Tree:" <> (show (SV.length $ fst3  (slimAlignment parentChar), SV.length $ fst3 finalGapped,isLeft, (slimAlignment parentChar), (slimGapped parentChar) ,(slimGapped childChar))) <> "->" <> (show finalGapped)) (
                                                                                                    if staticIA
                                                                                                        then M.makeIAFinalCharacter finalMethod charInfo childChar parentChar
                                                                                                        else
                                                                                                            childChar
                                                                                                                { slimFinal = finalAssignmentDO
                                                                                                                    ...
                                                                                                                    finalAssignmentDO =
                                                                                                                        if finalMethod == DirectOptimization
                                                                                                                            then
                                                                                                                                ...
                                                                                                                                in  snd3 finalAssignmentDONonGapped
                                                                                                                            else removeGapAndNil $ extractMedians finalGapped
                                                                                                                in  if staticIA
                                                                                                                        then M.makeIAFinalCharacter finalMethod charInfo childChar parentChar
                                                                                                                        else
                                                                                                                            childChar
                                                                                                                                { hugeFinal = finalAssignmentDO
                            ...
                            else -- display tree indegree=outdegree=1
                            ...
                            -- churrently using childChild
                                ...
                                if isIn1Out1
                                    then -- trace ("InOut1 preorder") (

                                        if localCharType == Add
                                            then childChar{rangeFinal = rangeFinal parentChar}
                                            else
                                                if localCharType == NonAdd
                                                    then childChar{stateBVFinal = stateBVFinal parentChar}
                                                    else
                                                        if localCharType `elem` packedNonAddTypes
                                                            then childChar{packedNonAddFinal = packedNonAddFinal parentChar}
                                                            else
                                                                if localCharType == Matrix
                                                                    then childChar{matrixStatesFinal = matrixStatesFinal parentChar}
                                                                    else
                                                                        if localCharType == AlignedSlim
                                                                            then childChar{alignedSlimFinal = alignedSlimFinal parentChar}
                                                                            else
                                                                                if localCharType == AlignedWide
                                                                                    then childChar{alignedWideFinal = alignedWideFinal parentChar}
                                                                                    else
                                                                                        if localCharType == AlignedHuge
                                                                                            then childChar{alignedHugeFinal = alignedHugeFinal parentChar}
                                                                                            else -- need to set both final and alignment for sequence characters

                                                                                                if (localCharType == SlimSeq) || (localCharType == NucSeq) -- parentChar
                                                                                                    then -- traceNoLF ("TNFinal-1/1") $
                                                                                                    -- trace ("TNFinal-1/1:" <> (show (isLeft, (slimAlignment parentChar), (slimGapped parentChar) ,(slimGapped childChar)))) $

                                                                                                        if staticIA
                                                                                                            then childChar{slimIAFinal = slimIAFinal parentChar}
                                                                                                            else
                                                                                                                childChar
                                                                                                                    { slimFinal = slimFinal parentChar
                                                                                                                    , slimAlignment -- slimAlignment parentChar
                                                                                                                      =
                                                                                                                        if isTree
                                                                                                                            then slimAlignment parentChar -- finalGappedO -- slimAlignment parentChar -- finalGappedO-- slimAlignment parentChar
                                                                                                                            else mempty
                                                                                                                    ...
                                                                                                                    , slimIAFinal -- slimFinal parentChar
                                                                                                                      =
                                                                                                                        if isTree
                                                                                                                            then slimFinal parentChar
                                                                                                                            else mempty
                                                                                                                    }
                                                                                                    else
                                                                                                        if (localCharType == WideSeq) || (localCharType == AminoSeq) -- parentChar
                                                                                                        -- trace ("TNFinal-1/1:" <> (show (isLeft, (slimAlignment parentChar), (slimGapped parentChar) ,(slimGapped childChar)))) (
                                                                                                            then
                                                                                                                if staticIA
                                                                                                                    then childChar{wideIAFinal = wideIAFinal parentChar}
                                                                                                                    else
                                                                                                                        childChar
                                                                                                                            { wideFinal = wideFinal parentChar
                                                                                                                            , wideAlignment -- wideAlignment parentChar
                                                                                                                              =
                                                                                                                                if isTree
                                                                                                                                    then wideAlignment parentChar -- finalGappedO -- wideAlignment parentChar -- finalGappedO-- wideAlignment parentChar
                                                                                                                                    else mempty
                                                                                                                            ...
                                                                                                                            , wideIAFinal -- wideFinal parentChar
                                                                                                                              =
                                                                                                                                if isTree
                                                                                                                                    then wideFinal parentChar
                                                                                                                                    else mempty
                                                                                                                            }
                                                                                                            else -- )

                                                                                                                if localCharType == HugeSeq -- parentChar
                                                                                                                -- trace ("TNFinal-1/1:" <> (show (isLeft, (hugeAlignment parentChar), (hugeGapped parentChar) ,(hugeGapped childChar)))) (
                                                                                                                    then
                                                                                                                        if staticIA
                                                                                                                            then childChar{hugeIAFinal = hugeIAFinal parentChar}
                                                                                                                            else
                                                                                                                                childChar
                                                                                                                                    { hugeFinal = hugeFinal parentChar
                                                                                                                                    , hugeAlignment -- hugeAlignment parentChar
                                                                                                                                      =
                                                                                                                                        if isTree
                                                                                                                                            then hugeAlignment parentChar -- finalGappedO -- hugeAlignment parentChar -- finalGappedO-- hugeAlignment parentChar
                                                                                                                                            else mempty
                                                                                                                                    ...
                                                                                                                                    , hugeIAFinal -- hugeFinal parentChar
                                                                                                                                      =
                                                                                                                                        if isTree
                                                                                                                                            then hugeFinal parentChar
                                                                                                                                            else mempty
                                                                                                                                    }
                                                                                                                    else -- )

                                                                                                                        error ("Unrecognized/implemented character type: " <> show localCharType)
                                    else -- )
                                    ...
                                    -- for Hardwired graphs
                                        ...
                                        if isIn2Out1
                                            then
                                                if isNothing parent2CharM
                                                    then error "Nothing parent2char in setFinal"
                                                    else -- trace ("SF: " <> "makeing 3-way final")
...
doPreOrderWithParentCheck
    ∷ (FiniteBits e, GV.Vector v e) ⇒ Bool → (v e, v e, v e) → (v e, v e, v e) → (v e, v e, v e) → (v e, v e, v e)
doPreOrderWithParentCheck isLeft alignmentParent gappedParent gappedChild =
    if not $ GV.null $ extractMediansGapped alignmentParent
        then --    if True then
...
makeNonGappedLeftRight charInfo gappedLeftRight nodeChar =
    let localCharType = charType charInfo
    in  if localCharType `elem` [SlimSeq, NucSeq]
            then
                ...
                in  (M.makeDynamicCharacterFromSingleVector newFinalGapped, mempty, mempty)
            else
                if localCharType `elem` [AminoSeq, WideSeq]
                    then
                        ...
                        in  (mempty, M.makeDynamicCharacterFromSingleVector newFinalGapped, mempty)
                    else
                        if localCharType == HugeSeq
                            then
                                ...
                                in  (mempty, mempty, M.makeDynamicCharacterFromSingleVector newFinalGapped)
...
-}
additivePreorder
    ∷ (V.Vector (Int, Int), V.Vector (Int, Int), V.Vector (Int, Int)) → V.Vector (Int, Int) → V.Vector (Int, Int)
additivePreorder (leftChild, nodePrelim, rightChild) parentFinal =
    if null nodePrelim
        then mempty
        else V.zipWith4 makeAdditiveCharacterFinal nodePrelim leftChild rightChild parentFinal
...
-}
nonAdditivePreorder
    ∷ (V.Vector BV.BitVector, V.Vector BV.BitVector, V.Vector BV.BitVector) → V.Vector BV.BitVector → V.Vector BV.BitVector
nonAdditivePreorder (leftChild, nodePrelim, rightChild) parentFinal =
    if null nodePrelim
        then mempty
        else V.zipWith4 makeNonAdditiveCharacterFinal nodePrelim leftChild rightChild parentFinal
...
-}
matrixPreorder
    ∷ Bool → V.Vector (V.Vector MatrixTriple) → V.Vector (V.Vector MatrixTriple) → V.Vector (V.Vector MatrixTriple)
matrixPreorder isLeft nodePrelim parentFinal =
    if null nodePrelim
        then mempty
        else V.zipWith (makeMatrixCharacterFinal isLeft) nodePrelim parentFinal
...
makeAdditiveCharacterFinal nodePrelim leftChild rightChild parentFinal =
    ...
    in  -- trace (show inData) (
        -- Rule 1
        if interNodeParent == Just parentFinal
            then -- trace ("R1 " <> show parentFinal)
                parentFinal
            else -- Rule 2

                if isJust ((leftChild `intervalUnion` rightChild) `intervalIntersection` parentFinal)
                    then
                        let xFactor = ((leftChild `intervalUnion` rightChild) `intervalUnion` nodePrelim) `intervalIntersection` parentFinal
                        in  if isNothing xFactor
                                then
                                    error
                                        ("I don't think this should happen in makeAdditiveCharacterFinal" <> show (nodePrelim, leftChild, rightChild, parentFinal))
                                else
                                    if isJust (fromJust xFactor `intervalIntersection` nodePrelim)
                                        then -- trace ("R2a " <> show (fromJust xFactor))
                                            fromJust xFactor
                                        else -- trace ("Rb " <> show (lciClosest (fromJust xFactor) nodePrelim))
                        ...
                        in  -- trace ("R3 " <> show (min closestPtoA closestULRtoA, max closestPtoA closestULRtoA))
...
makeNonAdditiveCharacterFinal nodePrelim leftChild rightChild parentFinal =
    ...
    -- trace (show inData) (
    if BV.isZeroVector (complement nodePrelim .&. parentFinal)
        then -- trace ("R1 " <> show parentFinal)
            parentFinal
        else
            if BV.isZeroVector (leftChild .&. rightChild)
                then -- trace ("R2 " <> show (nodePrelim .|. parentFinal))
                    nodePrelim .|. parentFinal
                else -- trace ("R3 " <> show (nodePrelim .|.  (leftChild .&. parentFinal) .|. (rightChild .&. parentFinal)))
...
makeMatrixCharacterFinal isLeft nodePrelim parentFinal =
    let numStates = length nodePrelim
        ...
        allThree =
            if isLeft
                then V.zip3 stateCostList stateLeftChildList stateIndexList
...
setCostsAndStates bestPrelimStates leftChildState rightChildStates stateIndex =
    if stateIndex `elem` bestPrelimStates
        then (stateIndex, leftChildState, rightChildStates)
...
setMinCostStatesMatrix numStates inStateVect =
    let outStates =
            V.filter ((/= (maxBound ∷ StateCost)) . fst3)
                <$> fmap (nonMinCostStatesToMaxCost (V.fromList [0 .. (numStates - 1)])) inStateVect
    in  outStates
...
nonMinCostStatesToMaxCost stateIndexList tripleVect =
    ...
    in  -- trace ((show stateIndexList) <> " " <> (show $ V.zip tripleVect stateIndexList))
        result
    where
...
setFinalToPreliminaryStates inVertBlockData =
    if V.null inVertBlockData
        then mempty
        else fmap setBlockFinalToPrelim inVertBlockData
...
setFinalToPrelimCharacterData inChar =
    inChar
        { stateBVFinal = snd3 $ stateBVPrelim inChar
...
setPreliminaryToFinalStates inVertBlockData =
    if V.null inVertBlockData
        then mempty
        else fmap setBlockPrelimToFinal inVertBlockData
...
setPrelimToFinalCharacterData inChar =
    inChar
        { stateBVPrelim = (stateBVFinal inChar, stateBVFinal inChar, stateBVFinal inChar)
