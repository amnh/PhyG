{- |
Module specifying post-order softwiired graph functions
-}
                    ...
                    multiTraverseTripleList ←
                        if (multiTraverseCharacters inGS == True)
                            then do
                                ...
                                pure tripleResult
                            else -- PU.seqParMap (parStrategy $ lazyParStrat inGS) (getTreeTriple rootIndex) multiTraverseTreeList
                            do
                                triplePar ← getParallelChunkMap
...
getDisplayBasedRerootSoftWired' inGS inGraphType rootIndex inPhyloGraph@(a, b, decGraph, _, _, f) =
    if LG.isEmpty (fst6 inPhyloGraph)
        then pure inPhyloGraph
            ...
            rerootResult ←
                rerootPar
                    (rerootBlockCharTrees' inGS rootIndex)
                    (zip3 (V.toList $ fmap head inBlockGraphV) (V.toList inBlockCharGraphVV) (V.toList charInfoVV))
...
-}
rerootCharacterTree'
    ∷ LG.Node → [LG.Node] → CharInfo → VertexCost → DecoratedGraph → DecoratedGraph → (DecoratedGraph, VertexCost)
                                ...
                                if length nodeChildren == 1
                                    ...
                                    -- takes characters in blocks--but for tree really all same block
                                        ...
                                        let -- this ensures that left/right choices are based on leaf BV for consistency and label invariance
                                            ...
                                            newCharData =
                                                if staticIA
                                                    then createVertexDataOverBlocksStaticIA (vertData leftChildLabel) (vertData rightChildLabel) blockCharInfo []
                    ...
                    if isNothing blockInfo
                        then
                            ...
                            in  do
                                    ...
                                    lowestCostDisplayTree : _ ← GO.selectGraphsFull Best 1 0.0 multiTraverseTreeList
            ...
            if LG.isTree $ fst6 inGraph
                then pure 0.0
                    ...
                    let -- (bestTreeList, _) = extractLowestCostDisplayTree startVertex inGraph
                        ...
                        rootIndex = fromMaybe (fst . head . LG.getRoots $ fst6 inGraph) startVertex
