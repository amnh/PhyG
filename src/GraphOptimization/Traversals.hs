...
Extra input MaybeGraph (existingGraph) for incremental optimization for initial post-order 
...
generalizedGraphPostOrderTraversal inGS sequenceChars inData existingGraph leafGraph staticIA startVertex inSimpleGraph = do
    ...
    outgroupRooted ←
        if (graphType inGS) `elem` [Tree, HardWired]
            then POSW.postOrderTreeTraversal inGS inData existingGraph leafGraph staticIA startVertex inSimpleGraph
            else
                if (graphType inGS) == SoftWired
                    then POSW.postOrderSoftWiredTraversal inGS inData existingGraph leafGraph staticIA startVertex inSimpleGraph
