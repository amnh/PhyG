    ...
    if isNothing inSimAnnealParams
        then do
            ...
            in  do
                    ...
                    graphs ← GO.selectGraphs Best numToKeep 0.0 $ concat annealRoundsList
            ...
            if counter == maxRounds
                ...
                do
                    ...
                    (insertGraphList', insertGraphCost, toDeleteList) ← case insertGraphList of
                        ...
                        gs → do
                            newList ← GO.selectGraphs Best (maxBound ∷ Int) 0.0 gs
                    ...
                    (newBestGraphList, newBestGraphCost, graphsToDoNext) ← case deleteGraphList of
                        [] → pure (curBestGraphList, curBestGraphCost, inPhyloGraphList)
                        gs → do
                            newDeleteGraphs ← GO.selectGraphs Best (maxBound ∷ Int) 0.0 gs
                    ...
                    in  do
                            ...
                            netEdgeList ←
                                if not doRandomOrder
                                    then pure $ LG.labNetEdges (thd5 firstPhyloGraph)
                                    else (`permuteList` (LG.labNetEdges $ thd5 firstPhyloGraph)) <$> getRandom

                            -- TODO
                            pTraverse ← getParallelChunkTraverse
                            deleteResult ← pTraverse action (fmap LG.toEdge netEdgeList)
                            let newGraphList' = concat deleteResult
                            newGraphList ← GO.selectGraphs Best numToKeep 0.0 newGraphList'
                            let newGraphCost = case newGraphList' of
                                    [] → infinity
                                    _ : _ → snd5 $ head newGraphList
                                    ...
                                    if isNothing inSimAnnealParams
                                        ...
                                        --    else
                                            ...
                                            in  do
                                                    ...
                                                    uniqueGraphList ← GO.selectGraphs Unique numToKeep 0.0 newGraphList'
                                                    let annealBestCost
                                                            | not $ null uniqueGraphList = min curBestGraphCost . snd5 $ head uniqueGraphList
                                                            | otherwise = curBestGraphCost
                                                    ...
                                                    (acceptFirstGraph, newSAParams) ←
                                                        if (not . null) uniqueGraphList
                                                            ...
                                                            else pure (False, U.incrementSimAnnealParams inSimAnnealParams)
                                                        ...
                                                        then -- this fixes tail fail
                                                            ...
                                                            if returnMutated
                                                                ...
                                                                do
                                                                    ...
                                                                    bestMoveList ← GO.selectGraphs Best numToKeep 0.0 bestMoveList'
            ...
            then -- check for multiple rounds of addition--if > 1 then need to randomize order
                ...
                if maxRounds == 1
                    then
                        ...
                        graphs ← GO.selectGraphs Best numToKeep 0.0 $ concat insertGraphList
                        pure (graphs, sum counterList)
            else
                let -- create list of params with unique list of random values for rounds of annealing
                    annealingRounds = rounds $ fromJust inSimAnnealParams
                    annealParamGraphList = U.generateUniqueRandList annealingRounds inSimAnnealParams
                in  -- replicateRandIntList = fmap randomIntList (take annealingRounds
                    do
                        -- TODO
                        ...
                        insertGraphResult ←
                            actionPar action (zip annealParamGraphList (replicate annealingRounds inPhyloGraphList))
                        ...
                        graphs ← GO.selectGraphs selectionType numToKeep 0.0 $ concat annealRoundsList
                        pure (graphs, sum counterList)
    ...
    if null inPhyloGraphList
        ...
        -- and don't want mutated--straight deletion on current best graphs
            ...
            in  do
                    ...
                    bestNewGraphList ← GO.selectGraphs Best numToKeep 0.0 newGraphList
                    let newGraphCost
                            | (not . null) bestNewGraphList = snd5 $ head bestNewGraphList
                            | otherwise = infinity
        ...
        if newGraphCost < curBestGraphCost
            then do
                ...
                if ((currentStep $ fromJust inSimAnnealParams) >= (numberSteps $ fromJust inSimAnnealParams))
                    ...
                    then do
                        graphs ← GO.selectGraphs Unique numToKeep 0.0 $ newGraphList <> curBestGraphList
                        pure $ (graphs, counter)
                    else -- more to do
...
postProcessNetworkAdd
    ...
    → [ReducedPhylogeneticGraph]
    → PhyG ([ReducedPhylogeneticGraph], Int)
postProcessNetworkAdd inGS inData maxNetEdges numToKeep counter returnMutated doSteepest doRandomOrder (curBestGraphList, _) (newGraphList, newGraphCost) inSimAnnealParams _ currentCost inPhyloGraphList =
        ...
        then error "Empty input insertEachNetEdge graph in deleteAllNetEdges"
            ...
            in  do
                    ...
                    candidateNetworkEdgeList ←
                        if doRandomOrder
                            then (`permuteList` candidateNetworkEdgeList') <$> getRandom
                        ...
                        then do
                            ...
                            if null candidateNetworkEdgeList
                                then do
                                    ...
                                    if doSteepest
                                        then do
                                            graphs ← GO.selectGraphs Best numToKeep 0.0 newGraphList
                                            --  trace ("IENE: All " <> (show minCost))
                                            pure (graphs, minCost, newSAParams)
                                                ...
                                                then
                                                    ...
                                                    in  if minCost < currentCost
                                                            then do
                                                                ...
                                                                insertResult ← insertPar insertAction $ zip annealParamList newGraphList
                                                                ...
                                                                (allMinCost, allMinCostGraphs) ←
                                                                    if (not . null . concat) allGraphListList
                                                                        then GO.selectGraphs Unique numToKeep 0.0 (concat allGraphListList) >>= \gs → pure (minimum costList, gs)
                                                                        else pure (infinity, [])

                                                                pure (allMinCostGraphs, allMinCost, U.incrementSimAnnealParams $ head allSAParamList)
                                                            else do
                                                                graphs ← GO.selectGraphs Unique numToKeep 0.0 newGraphList
                                                                pure (graphs, minCost, newSAParams)
                                                        ...
                                                        then do
                                                            ...
                                                            if ((currentStep $ fromJust inSimAnnealParams) >= (numberSteps $ fromJust inSimAnnealParams))
                                                                ...
                                                                do
                                                                    ...
                                                                    if acceptGraph
                                                                        then pure (newGraphList, minCost, newSAParams)
                                                                        else
                                                                            insertEachNetEdge
                                                                                inGS
                                                                                inData
    ...
    if null inEdgePairList
        then do
            ...
            in  do
                    ...
                    newGraphList ← GO.selectGraphs Best (maxBound ∷ Int) 0.0 newGraphList'
                                                ...
                                                do
                                                    ...
                                                    if ((currentStep $ fromJust inSimAnnealParams) >= (numberSteps $ fromJust inSimAnnealParams))
                                                        ...
                                                        do
                                                            ...
                                                            if acceptGraph
                                                                then pure (newGraphList, nextSAParams)
                                                                else
                                                                    insertNetEdgeRecursive
                                                                        inGS
                                                                        inData
    ...
    if isNothing inSimAnnealParams
        then
            ...
            in  do
                    -- TODO
                    actionPar ← getParallelChunkTraverse
                    deleteResult ← actionPar action (zip annealParamGraphList (replicate annealingRounds inPhyloGraphList))
            ...
            in  do
                    networkEdgeList ←
                        if not doRandomOrder
                            ...
                            else (`permuteList` networkEdgeList') <$> getRandom
                    ...
                    bestCostGraphList ← filter ((/= infinity) . snd5) <$> GO.selectGraphs Best numToKeep 0.0 newGraphList
                    let minCost =
                                        ...
                                        then
                                            if minCost < currentCost
                                                then -- trace ("DENE--Delete net edge return:" <> (show (minCost,length uniqueCostGraphList))) (
                                                    ...
                                                    let annealParamList = U.generateUniqueRandList (length bestCostGraphList) newSAParams
                                                    ...
                                                    in  do
                                                            ...
                                                            graphs ← GO.selectGraphs Unique numToKeep 0.0 newGraphListBetter
                                                            pure (graphs, newMinCost, newSAParams)
                                                else do
                                                ...
                                                then do
                                                    ...
                                                    if ((currentStep $ fromJust inSimAnnealParams) >= (numberSteps $ fromJust inSimAnnealParams))
                                                        ...
                                                        do
                                                            (acceptGraph, nextSAParams) ← U.simAnnealAccept inSimAnnealParams currentCost minCost
                                                            if acceptGraph
                                                                then pure (bestCostGraphList, minCost, newSAParams)
                                                                else deleteEachNetEdge inGS inData numToKeep doSteepest doRandomOrder force nextSAParams inPhyloGraph
    ...
    if null inEdgeToDeleteList
        then do
            ...
            in  do
                    ...
                    newPhyloGraphList ← GO.selectGraphs Best (maxBound ∷ Int) 0.0 newPhyloGraphList'
                                        ...
                                        -- if better always accept
                                            ...
                                            if (snd5 $ head newPhyloGraphList) < (snd5 inPhyloGraph)
                                                then -- these graph costs are "exact" or at least non-heuristic--needs to be updated when get a good heuristic
                                                    ...
                                                    (\(_, x) → (newPhyloGraphList, x)) <$> U.simAnnealAccept inSimAnnealParams (snd5 inPhyloGraph) (snd5 $ head newPhyloGraphList)
                                                else -- check if hit step limit--more for SA than drift
                                                    ...
                                                    if ((currentStep $ fromJust inSimAnnealParams) >= (numberSteps $ fromJust inSimAnnealParams))
                                                        ...
                                                        do
                                                            ...
                                                            if acceptGraph
                                                                then pure (newPhyloGraphList, nextSAParams)
                                                                else deleteNetEdgeRecursive inGS inData inPhyloGraph force nextSAParams (drop numGraphsToExamine inEdgeToDeleteList)
