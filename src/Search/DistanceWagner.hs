...
{- |
Module with distance tree construction methods Distance Wagner Farris 1972
...
getStartingPair choiceOpt distMatrix
    ...
    | otherwise =
        errorWithoutStackTrace ("Initial pair option " <> choiceOpt <> " unrecognized.  Must be 'closest', 'furthest', or 'random'")


-- | getBestEdgeTree take list of edge tuples and return trhe one with best addition cost
getBestEdgeTree
    ∷ V.Vector (Double, Tree, M.Matrix Double) → Double → (Double, Tree, M.Matrix Double) → (Double, Tree, M.Matrix Double)
getBestEdgeTree edgeTreeList curBestCost curBestResult =
    if V.null edgeTreeList
        then curBestResult
        else
            let (firstAddCost, _, _) = V.head edgeTreeList
            in  if firstAddCost < curBestCost
                    then getBestEdgeTree (V.tail edgeTreeList) firstAddCost (V.head edgeTreeList)
...
addToEdgeSwap distMatrix leaf initialTree newLeafIndex inEdge =
    let (eVertex, uVertex, inWeight) = inEdge
        ...
        newEdgeVect = cleanupEdges <> newEdges
    ...
    in  (addCost + eVertLeafDist + uVertLeafDist - inWeight, newTree, augmentedDistMatrix)
...
addToEdge distMatrix leaf initialTree newLeafIndex inEdge =
    ...
    let (eVertex, uVertex, _) = inEdge
        ...
        newEdges = V.fromList [(leaf, newLeafIndex, addCost), (eVertex, newLeafIndex, eVertLeafDist), (uVertex, newLeafIndex, uVertLeafDist)]
        cleanupEdges = V.filter (/= inEdge) initialEdgeVect
        newEdgeVect = V.map orderEdge $ cleanupEdges <> newEdges
        newTree = (newVertexVect, newEdgeVect)
        -- add new costs from added vertex to each reamaining leaf
        augmentedDistMatrix = getNewDistMatrix distMatrix addCost eVertLeafDist uVertLeafDist eVertex uVertex leaf
    in  (addCost, newTree, augmentedDistMatrix)
...
addTaxonToTree distMatrix initialTree leavesToAdd newVertexIndex leaf =
    if V.null leavesToAdd
        then (0.0, initialTree, leavesToAdd, distMatrix)
        else
            ...
            in  -- filter for best addition point
                let (addCost, newTree, augmentedDistMatrix) = getBestEdgeTree (V.tail addEdgeList) firstAddCost (V.head addEdgeList)
                in  (addCost, newTree, leavesRemaining, augmentedDistMatrix)


-- | getBestLeafAdd chooses best leaf to add based on cost field
getBestLeafAdd
    ∷ V.Vector (Double, Tree, V.Vector Int, M.Matrix Double)
    → Double
    → (Double, Tree, V.Vector Int, M.Matrix Double)
    → (Double, Tree, V.Vector Int, M.Matrix Double)
getBestLeafAdd addPosVect curBestCost curBestLeaf =
    if V.null addPosVect
        then curBestLeaf
        else
            let (thisCost, _, _, _) = V.head addPosVect
            in  if thisCost < curBestCost
                    then getBestLeafAdd (V.tail addPosVect) thisCost (V.head addPosVect)
...
-}
wagBest
    ∷ M.Matrix Double → Tree → V.Vector Int → Int → Int → V.Vector Int → String → PhyG (Tree, V.Vector Int, M.Matrix Double)
wagBest distMatrix inTree leavesToAdd nOTUs newVertexIndex leavesToMap choiceOpt
    | length leavesToAdd == nOTUs =
        ...
        in  wagBest distMatrix initialTree leavesToAdd' nOTUs nOTUs leavesToAdd' choiceOpt
    ...
    | otherwise =
        ...
        in  let -- progress = takeWhile (/='.') $ show  ((fromIntegral (100 * (newVertexIndex - nOTUs))/fromIntegral (nOTUs - 2)) :: Double)
...
calculateWagnerTrees distMatrix choiceOpt =
    if M.dim distMatrix == (0, 0)
        then errorWithoutStackTrace "Null distance matrix"
        else -- get initial pair of leaves and create initial tree
...
getRandomAdditionSequence leafNames distMatrix outgroup initiaLeavesToAdd =
    ...
    in  let eVertex = initiaLeavesToAdd V.! 0
        ...
        in  let thisTree = makeTreeFromOrder distMatrix initialTree nOTUs nOTUs leavesToAdd
            ...
            in  (newickTree', fst thisTree, treeCost, snd thisTree)
                ...
                if addSequence == "asis"
                    then
                        let initialTree = (V.fromList [0, 1], V.fromList [(0, 1, distMatrix M.! (0, 1))])
                            ...
                            newickTree' = take (length newickTree - 3) newickTree <> "[" <> showDouble precision treeCost <> "]" <> ";"
                        in  return $ [(newickTree', fst asIsResult, treeCost, snd asIsResult)]
                    else
                        if head addSequence == 'r'
                            then
                                if null replicateSequences
                                    then errorWithoutStackTrace "Zero replicate additions specified--could be error in configuration file"
                                    else
                                        if (length replicateSequences > 10000)
                                            then doWagnerRASProgressive inGS leafNames distMatrix outgroup numToKeep [] replicateSequences
...
-}
doWagnerRASProgressive
    ∷ GlobalSettings → V.Vector String → M.Matrix Double → Int → Int → [TreeWithData] → [V.Vector Int] → PhyG [TreeWithData]
doWagnerRASProgressive inGS leafNames distMatrix outgroup numToKeep curBestTreeList replicateSequences =
    if null replicateSequences
        then pure curBestTreeList
        else
...
getSubEdges inVertexList nOTUs edgeVect subEdgeVect howMany
    ...
    | otherwise =
        ...
        in  if V.null foundVect
                then getSubEdges (tail inVertexList) nOTUs edgeVect subEdgeVect howMany -- only terminals left
                else
                    if V.length foundVect /= 2
                        then
                            error
                                ( "Index ("
                                    <> howMany
                                    ...
                                    <> show startVertex
                                    <> "->found "
                                    <> show (V.length foundVect)
                                    <> " but should be two edges in "
                                    <> show foundVect
                                    <> " in "
                                    <> show edgeVect
                                )
...
adjustVertex (inE, inU, w) hV lV nOTUs
    ...
    | lV < nOTUs -- update pendant edge was deleted since hV > lV; hV must be > nOTUs
        =
        if (inE > max hV nOTUs) && (inU > max hV nOTUs)
            then (inE - 1, inU - 1, w)
            else
                if (inE <= max hV nOTUs) && (inU > max hV nOTUs)
                    then (inE, inU - 1, w)
...
updateVertexNUmbersOnEdges eVert uVert edgeList nOTUs =
    ...
    if V.null edgeList
        then V.empty
            ...
            in  V.cons newVertex (updateVertexNUmbersOnEdges eVert uVert (V.tail edgeList) nOTUs)
...
getEndVertices inEdges index =
    if V.length inEdges /= 2
        then error ("Edge number should be 2 not " <> show (V.length inEdges) <> " in getEndVertices")
        else
            let (fEVertex, fUVertex, _) = V.head inEdges
                (sEVertex, sUVertex, _) = V.last inEdges
            in  if fEVertex == index
                    then
                        if sEVertex == index
                            then (fUVertex, sUVertex)
                            else (fUVertex, sEVertex)
                    else
                        if fUVertex == index
                            then
                                if sEVertex == index
                                    then (fEVertex, sUVertex)
                                    else (fEVertex, sEVertex)
...
getContractedWeight aVert bVert distMatrix nOTUs edgeVect
    ...
    | otherwise =
        ...
        in  max firstEstimate secondEstimate / 2.0
...
contractEdges distMatrix nOTUs edgeVect index allEdges
    ...
    | otherwise =
        ...
        in  orderEdge (aVert, bVert, newWeight)
...
getNewDistMatrix origDist addCost eVertLeafDist uVertLeafDist eVertex uVertex leafIndex =
    ...
    in  M.addMatrixRow origDist newDistRow'
...
enterNewEdgeCost columnNumber edgeVect rowNumber =
    if V.null edgeVect
        then NT.infinity -- not found so Infty
        else
            let (a, b, weight) = V.head edgeVect
            in  if (columnNumber == a && rowNumber == b) || (columnNumber == b && rowNumber == a)
                    then weight
                    else enterNewEdgeCost columnNumber (V.tail edgeVect) rowNumber
...
getNewDistMatrixInternal inMatrix newEdgeVect =
    ...
    if V.length newEdgeVect /= 5
        then error ("Wrong size edgeVector shoud be 5 and is " <> show (V.length newEdgeVect))
        else
            ...
            in  M.addMatrices inMatrix (V.fromList [V.fromList newDistColumnI, V.fromList newDistColumnII])
...
connectEdges distMatrix eEdges uEdges edgesToConnect
    ...
    | otherwise =
        let edgesKept = subtractVector edgesToConnect eEdges <> subtractVector edgesToConnect uEdges
        ...
        in  (centralEdgeCost + aCost + bCost + cCost + dCost - wAB - wCD, (V.empty, newEdgeVect <> edgesKept), newDistMatrix)
...
-}
addEdgeToSplit
    ∷ V.Vector Edge → V.Vector Edge → Edge → Edge → M.Matrix Double → V.Vector Edge → (Double, Tree, M.Matrix Double)
...
splitTree distMatrix inTree inTreeCost edgeToRemove =
    ...
    let (_, edgeVect) = inTree
        ...
        -- newEdgeSet = subtractVector (V.cons edgeToRemove $ eEdges <> uEdges) edgeVect
    ...
    in  if eVertex < nOTUs
            then (V.singleton (eVertex, eVertex, 0.0), uSubEdges'', delta, previousEdges'', distMatrix'') -- this so know a pendant edge
            else
                if uVertex < nOTUs
                    then (V.singleton (uVertex, uVertex, 0.0), eSubEdges'', delta, previousEdges'', distMatrix'') -- this so know a pendant edge
                    else -- neded to have e first then u for SPR/TBR so can coordinate with previous edges
...
-}
sieveTrees
    ∷ Double → Double → V.Vector (Double, Tree, M.Matrix Double) → V.Vector String → Int → [TreeWithData] → [TreeWithData]
sieveTrees inDelta curBestCost inAddList leafNames outgroup savedTrees =
    if null inAddList
        then savedTrees
            ...
            in  if firstDelta > inDelta
                    then sieveTrees inDelta curBestCost (V.tail inAddList) leafNames outgroup savedTrees
                    else
                        if newCost < curBestCost
                            then sieveTrees inDelta curBestCost (V.tail inAddList) leafNames outgroup [newTuple]
                            else
                                if withinEpsilon newCost curBestCost
                                    then sieveTrees inDelta curBestCost (V.tail inAddList) leafNames outgroup (newTuple : savedTrees)
...
reAddTerminals rejoinType curBestCost leafNames outGroup split =
    if rejoinType /= "otu"
        then error ("Incorrect swap function in reAddTerminals: " <> rejoinType)
        else
            let (eEdgeVect, uEdgeVect, delta, _, distMatrix) = split
                nOTUs = V.length leafNames
            in  if (V.length eEdgeVect > 1)
                    || ((fst3 (V.head eEdgeVect) /= snd3 (V.head eEdgeVect)) && (fst3 (V.head eEdgeVect) < nOTUs) && (snd3 (V.head eEdgeVect) < nOTUs))
                    then []
                    else
                        ( if M.rows distMatrix /= ((2 * nOTUs) - 3)
                            then error ("Dist Matrix incorrect size " <> show (M.dim distMatrix) <> " should be " <> show ((2 * nOTUs) - 3, (2 * nOTUs) - 3))
                            else
...
addToEdgeSwapRecurse inDelta distMatrix leaf initialTree newLeafIndex inEdgeVect =
    if V.null inEdgeVect
        then (inDelta, initialTree, distMatrix)
        else
            ...
            in  if newDelta < inDelta
                    then (newDelta, newTree, augmentedDistMatrix)
...
getVectorAllVectorPairs firstVect secondVect =
    if V.null firstVect
        then V.empty
            ...
            in  firstPairs <> getVectorAllVectorPairs (V.tail firstVect) secondVect
...
createVectorEdgePairs pairSet previousEdges eEdgeVect uEdgeVect
    | pairSet == "spr" =
        ...
        in  V.map (V.cons eEdgePrev) $ V.map V.singleton uEdgeVect
...
addEdgeToSplitRecurse
    ∷ V.Vector Edge
    → V.Vector Edge
    → Edge
    → Edge
    → M.Matrix Double
    → V.Vector (V.Vector Edge)
    → (Double, Tree, M.Matrix Double)
    → (Double, Tree, M.Matrix Double)
addEdgeToSplitRecurse eEdges uEdges eTerminal uTerminal distMatrix edgesToConnectVect origTriple@(inDelta, _, _) =
    if V.null edgesToConnectVect
        then origTriple
        else
            let edgesToConnect = V.head edgesToConnectVect
            in  if V.null eEdges && V.null uEdges
                    then error "Empty e/u Edge vectors in addEdgeToSplit"
                    else
                        if V.null edgesToConnect
                            then error "Empty eEdge vector in addEdgeToSplit"
                            else
                                if fst3 (V.head eEdges) == (-1) -- adding eOTU, V.last since the (-1,-1,0) edge should always be first
                                    then
...
doSPRTBR rejoinType curBestCost leafNames outGroup split =
    ...
    in  if V.null eEdgeVect || V.null uEdgeVect
            then error "Empty edge vectors in doSPRTBR"
            else
                if fst3 (V.head eEdgeVect) == snd3 (V.head eEdgeVect) -- if an OTU call readdTerminal
                    then
                        let newLeafIndex = M.rows distMatrix
                ...
                if fst3 (V.head eEdgeVect) == snd3 (V.head eEdgeVect) -- if an OTU call readdTerminal
                    then
                        let newLeafIndex = M.rows distMatrix
                            ...
                            newickTree' = take (length newickTree - 3) newickTree <> "[" <> showDouble precision newCost <> "]" <> ";"
                        in  if newCost < curBestCost
                                then (newickTree', newTree, newCost, newMatrix)
                                else origTree
                    else -- internal edge or edge with two OTUs as vertices
                    -- check to make sure edges are where they should be can remove later
                    -- fix e edge and join to each u edge for SPR (n^2)
...
filterNewTreesOnCost curBestCost firstTreeList savedTrees =
    if null firstTreeList
        then savedTrees
        else
            let firstTree = head firstTreeList
                (_, _, firstCost, _) = firstTree
            in  if firstCost < curBestCost
                    then filterNewTreesOnCost firstCost (tail firstTreeList) [firstTree]
                    else
                        if firstCost > curBestCost
                            then filterNewTreesOnCost curBestCost (tail firstTreeList) savedTrees
                            else
                                let uniqueTree = filterNewTrees savedTrees firstTree
                                in  if isNothing uniqueTree
                                        then filterNewTreesOnCost curBestCost (tail firstTreeList) savedTrees
...
splitJoin swapFunction refineType leafNames outGroup edgeVect curTreeWithData@(_, curTree, curTreeCost, curTreeMatrix) =
    if V.null edgeVect
        then curTreeWithData -- All splits tested, nothing better found
        else
            ...
            in  if firstTreeCost < curTreeCost
                    then splitJoin swapFunction refineType leafNames outGroup (snd firstNewTree) firstTree
...
splitJoinWrapper swapFunction refineType leafNames outGroup curTreeWithData@(_, curTree, _, _) =
    ...
    in  splitJoin swapFunction refineType leafNames outGroup edgeVect curTreeWithData
    ...
    if refinement == "none"
        then pure [inTree]
            ...
            if refinement == "otu"
                then do
                    newTrees ←
                        getGeneralSwapSteepestOne "otu" reAddTerminalsSteep leafNames outGroup [inTree] [([], (V.empty, V.empty), NT.infinity, M.empty)]
                    newTrees' ←
                        getGeneralSwap
                            "otu"
                            reAddTerminals
                            ...
                            outGroup
                            newTrees
                            [([], (V.empty, V.empty), NT.infinity, M.empty)]

                    if refinement == "best:1"
                        then
                            if not $ null newTrees
                                then pure newTrees
                                else pure [inTree]
                        else
                            if not (null newTrees')
                                then pure newTrees'
                                else -- trace "OTU swap did not find any new trees"
                                    pure [inTree]
                else
                    if refinement == "spr"
                        then do
                            newTrees ←
                                getGeneralSwapSteepestOne "otu" reAddTerminalsSteep leafNames outGroup [inTree] [([], (V.empty, V.empty), NT.infinity, M.empty)]
                            newTrees' ←
                                getGeneralSwapSteepestOne "spr" doSPRTBRSteep leafNames outGroup newTrees [([], (V.empty, V.empty), NT.infinity, M.empty)]
                            newTrees'' ←
                                getGeneralSwap
                                    "spr"
                                    doSPRTBR
                            ...
                            if saveMethod == "best:1"
                                then do
                                    ...
                                    if not (null newTrees'')
                                        then pure newTrees''
                                        else -- trace "SPR swap did not find any new trees"
                                            pure [inTree]
                        else
                            if refinement == "tbr"
                                then do
                                    ...
                                    newTrees' ←
                                        getGeneralSwapSteepestOne "spr" doSPRTBRSteep leafNames outGroup newTrees [([], (V.empty, V.empty), NT.infinity, M.empty)]
                                    newTrees'' ←
                                        getGeneralSwapSteepestOne "tbr" doSPRTBRSteep leafNames outGroup newTrees' [([], (V.empty, V.empty), NT.infinity, M.empty)]
                                    newTrees''' ←
                                        getGeneralSwap
                                            "tbr"
                                            doSPRTBR
                                    ...
                                    if saveMethod == "best:1"
                                        then do
                                            ...
                                            if not (null newTrees''')
                                                then pure newTrees'''
                                                else -- trace "TBR swap did not find any new trees"
                                                    pure [inTree]
