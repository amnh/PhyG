{- |
Module with useful functionsfor  distance tree construction methods dWag, Neightbor-Joining, UPGMA, and WPGMA --  but with added refinement based on 4-point metric
-}
...
localRound val places =
    ...
    in  fromIntegral roundedVal / factor
...
vertex2FGLNode leafVect vertIndex =
    if vertIndex < V.length leafVect
        then (vertIndex, T.pack (leafVect V.! vertIndex))
        else
            let vertexName = "HTU" <> show (vertIndex - V.length leafVect)
            in  (vertIndex, T.pack vertexName)
...
tree2FGL inTree@(inVertexVect, inEdgeVect) leafNameVect =
    if inTree == emptyTree
        then error "Empty tree in tree2FGL"
        else
            let fglNodes = V.map (vertex2FGLNode leafNameVect) inVertexVect
            in  -- trace ("tree2FGL orig, vertices, edges =  " <> (show $ length inVertexVect ) <> " "  <> (show $ length fglNodes ) <> " " <> (show $ length inEdgeVect ))
...
reIndexEdges inVertexList (e, u, w) =
    if null inVertexList
        then error "Null inVertexList in reIndexEdges"
        else
            let newE = L.elemIndex e inVertexList
                newU = L.elemIndex u inVertexList
            in  -- un safe not testing but list is created from edges first
...
makeVertexNames vertList nOTUs leafNames nameHTUs =
    if null vertList
        then []
            ...
            in  if firstVert < nOTUs
                    then (leafNames V.! firstVert) : makeVertexNames (tail vertList) nOTUs leafNames nameHTUs
                    else
                        if nameHTUs
                            then ("HTU" <> show firstVert) : makeVertexNames (tail vertList) nOTUs leafNames nameHTUs
...
getChildEdges vertIndex nLeaves inEdgeVect
    ...
    | otherwise =
        ...
        in  if (a == vertIndex) || (b == vertIndex)
                then V.cons (a, b, w) (getChildEdges vertIndex nLeaves (V.tail inEdgeVect))
                else getChildEdges vertIndex nLeaves (V.tail inEdgeVect)
...
directEdges vertIndex nLeaves isFirst inEdgeVect
    ...
    | isFirst -- to find out group edge order larger to smaller will have outgroup index second
        ...
        in  V.cons (a, b, w) (directEdges a nLeaves False remainingEdgeVect)
    ...
    | otherwise -- not outgroup but regular node, get two child edges
        ...
        in  if V.length newDescEdges /= 2
                then
                    error
                        ( "There should be 2 child edges for index "
                            <> show vertIndex
                            <> " and there are(is) "
                            <> show (V.length newDescEdges)
                            <> " "
                            <> show newDescEdges
                        )
                else
                    ...
                    in  (newDescEdges <> (firstSubEdges <> secondSubEdges))
...
convertToDirectedGraph leafList outgroupIndex inTree =
    ...
    in  G.mkGraph labelledVertexList edgeList
...
convertToNewick leafNames outGroup inTree
    ...
    | otherwise =
        ...
        in  -- trace ("ConverttoNewick in-vertices in-edges" <> (show $ length inVertexVect ) <> " "  <> (show $ V.toList inVertexVect ) <> "\n" <> (show $ length vertexVect ) <> " "  <> (show vertexVect ) <> "\n" <> (show $ length edgeVect ) <> " " <> show edgeVect <> "\n" <> show fglTree)
...
getEdgeRootIndex edgeIndex outgroup edgeVect =
    if V.null edgeVect
        then error "Root edge not found"
        else
            let (eVect, uVect, _) = V.head edgeVect
            in  if (eVect == outgroup) || (uVect == outgroup)
                    then (edgeIndex, V.head edgeVect)
...
removeCrap inString =
    if length inString == 1
        then inString
            ...
            in  if firstChar == ',' && secondChar == ','
                    then ',' : removeCrap (drop 2 inString)
                    else
...
convertToNewickGuts leafNames outGroup wagTree =
    ...
    in  let (firstVert, secondVert, weight) = foundEdge
            remainderEdges = V.filter (/= foundEdge) edgeVect
        in  -- this is embarassing bullshit  -- converting  ",,"  to ","
            if firstVert == outGroup
                then
                    "("
                        <> (leafNames V.! outGroup)
                        <> ":"
                        <> showDouble 8 (weight / 2.0)
                        <> ","
                        <> getEdgesNonRoot secondVert remainderEdges (V.length leafNames) leafNames
                        <> ":"
                        <> showDouble 8 (weight / 2.0)
                        <> ")"
                else
                    "("
                        ...
                        <> showDouble 8 (weight / 2.0)
                        <> ","
                        <> getEdgesNonRoot firstVert remainderEdges (V.length leafNames) leafNames
                        <> ":"
                        <> showDouble 8 (weight / 2.0)
                        <> ")"
...
getEdgeRoot edgeIndex edgeVect =
    if V.null edgeVect
        then (-1, -1, -1.0)
            ...
            let (eVect, uVect, _) = V.head edgeVect
            in  if (eVect == edgeIndex) || (uVect == edgeIndex) then V.head edgeVect else getEdgeRoot edgeIndex (V.tail edgeVect)
...
getEdgesNonRoot edgeIndex edgeVect nOTUs leafNames =
    ...
    in  if V.null edgeVect
            then "END"
            else
                let thisEdge = getEdgeRoot edgeIndex edgeVect
                in  if thisEdge == terminal
                        then "ERROR"
                        else
                            ...
                            in  if V.null remainderEdges
                                    then (leafNames V.! uVect) <> ":" <> showDouble precision weight <> ","
                                    else
                                        if eVect == edgeIndex
                                            then
                                                if eVect < nOTUs
                                                    then
                                                        if uDesc /= terminal
                                                            then "(" <> (leafNames V.! eVect) <> ":" <> showDouble precision weight <> "," <> uSubTree <> ")"
                                                            else (leafNames V.! eVect) <> ":" <> showDouble precision weight <> ","
                                                    else
                                                        if uVect < nOTUs
                                                            then
                                                                if eDesc /= terminal
                                                                    then "(" <> (leafNames V.! uVect) <> ":" <> showDouble precision weight <> "," <> eSubTree <> ")"
                                                                    else (leafNames V.! uVect) <> ":" <> showDouble precision weight <> ","
                                                            else
                                                                if (eDesc /= terminal) && (uDesc == terminal)
                                                                    then eSubTree <> ":" <> showDouble precision weight <> ","
                                                                    else
                                                                        if (eDesc == terminal) && (uDesc /= terminal)
                                                                            then uSubTree <> ":" <> showDouble precision weight <> ","
                                                                            else
                                                                                if length eSubTree < length uSubTree
                                                                                    then "(" <> eSubTree <> "," <> uSubTree <> ":" <> showDouble precision weight <> ")"
...
getBestTrees ∷ String → Int → [TreeWithData] → Double → [TreeWithData] → PhyG [TreeWithData]
getBestTrees keepMethod number inList curBestCost curBestTrees =
    if null inList
        then -- apply keep method if not keeping all

            if length curBestTrees <= number
                then pure curBestTrees
                else
                    if keepMethod == "first"
                        then pure $ take number curBestTrees
                        else
                            if keepMethod == "last"
                                then pure . reverse . take number $ reverse curBestTrees
                                else
                                    if keepMethod == "random"
                                        then -- not Fitch but shuffles elements and takes first n
                                            take number <$> shuffleList curBestTrees
                                        else failWithPhase Computing $ unwords ["Keep method", keepMethod, "not implemented"]
        else
            let firstTree = head inList
                (_, _, firstCost, _) = firstTree
            in  if firstCost < curBestCost
                    then getBestTrees keepMethod number (tail inList) firstCost [firstTree]
                    else
                        if withinEpsilon firstCost curBestCost
                            then getBestTrees keepMethod number (tail inList) firstCost (firstTree : curBestTrees)
...
getUniqueTrees inList uniqueList =
    if null inList
        then uniqueList
            ...
            in  if fstNewick `notElem` fmap fst4 uniqueList
                    then getUniqueTrees (tail inList) (firstTree : uniqueList)
...
-}
keepTrees ∷ [TreeWithData] → String → String → Double → PhyG [TreeWithData]
keepTrees inList saveMethod keepMethod curBestCost
    | null inList = pure []
    | saveMethod == "all" = pure inList
    | take 6 saveMethod == "unique" =
        if length saveMethod == 6
            then pure $ getUniqueTrees inList []
            else
                let number = read (drop 7 saveMethod) ∷ Int
                in  pure . take number . L.sortOn thd4 $ getUniqueTrees inList []
    | take 4 saveMethod == "best" =
        if length saveMethod == 4
            then (`getUniqueTrees` []) <$> getBestTrees keepMethod (maxBound ∷ Int) inList curBestCost []
            else
                if (saveMethod !! 4) == ':'
                    then
                        ...
                        in  getBestTrees keepMethod number saveTrees bestCost []
...
subtractVector a b
    ...
    | otherwise =
        ...
        in  if notFound
                then V.cons firstB (subtractVector a (V.tail b))
...
getVertexSet edgeVect =
    if V.null edgeVect
        then Set.empty
            ...
            in  Set.union thisSet (getVertexSet $ V.tail edgeVect)
...
getMinRowDistMatrix distMatrix tabuList minPair@(minCol, minVal) curColumn row
    ...
    | otherwise =
        ...
        in  if firstVal < minVal
                then getMinRowDistMatrix distMatrix tabuList (curColumn, firstVal) (curColumn + 1) row
...
getMatrixMinPairTabu distMatrix tabuList =
    if M.null distMatrix
        then error "Empty matrix in getMatrixPairTabu"
        else
...
getMatrixMinPair distMatrix curBest curRow curColumn
    ...
    | otherwise =
        let (_, _, currentBestDistance) = curBest
        in  if distMatrix M.! (curRow, curColumn) < currentBestDistance
                then getMatrixMinPair distMatrix (curRow, curColumn, distMatrix M.! (curRow, curColumn)) curRow (curColumn + 1)
...
getMatrixMaxPair distMatrix curBest curRow curColumn
    ...
    | otherwise =
        let (_, _, currentBestDistance) = curBest
        in  if distMatrix M.! (curRow, curColumn) > currentBestDistance
                then getMatrixMaxPair distMatrix (curRow, curColumn, distMatrix M.! (curRow, curColumn)) curRow (curColumn + 1)
