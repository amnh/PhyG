...
threeWaySlim charInfo parent1 parent2 curNode =
    let noChangeAdjust = False
        ...
        (median2, cost2) = get3WayGeneric (Measure.stateTransitionThreewayDispersion (slimTCM charInfo)) a2 b2 c2
...
threeWayWide charInfo parent1 parent2 curNode =
    let noChangeAdjust = False
        ...
        (median2, cost2) = get3WayGeneric (Measure.stateTransitionThreewayDispersion (wideTCM charInfo)) a2 b2 c2
...
threeWayHuge charInfo parent1 parent2 curNode =
    let noChangeAdjust = False
        ...
        (median2, cost2) = get3WayGeneric (Measure.stateTransitionThreewayDispersion (hugeTCM charInfo)) a2 b2 c2
...
{- |
get3WayGeneric takes /three/ vectors and produces a (median, cost) pair.
-}
get3WayGeneric ∷ (GV.Vector v e) ⇒ Measure.TCM3Dλ e → v e → v e → v e → (v e, StateChangeCost)
get3WayGeneric tcm in1 in2 in3 =
    let len = minimum $ GV.length <$> [in1, in2, in3]
        (vCost, vChar) = V.unzip . V.generate len $ \i → tcm (in1 GV.! i) (in2 GV.! i) (in3 GV.! i)
...
threeWayGeneric charInfo parent1 parent2 curNode =
    let noChangeAdjust = False
        ...
        (median1Slim, median1Wide, median1Huge, cost1) = case localCharType of
            t
                | t `elem` [SlimSeq, NucSeq] →
                    let (a, b, c) = addGapsToChildren (slimGapped p1p2cN) (slimGapped p1p2)
                        (median, cost) = get3WayGeneric (Measure.stateTransitionThreewayDispersion (slimTCM charInfo)) a b c
                    in  (median, mempty, mempty, cost)
            t
                | t `elem` [AminoSeq, WideSeq] →
                    let (a, b, c) = addGapsToChildren (wideGapped p1p2cN) (wideGapped p1p2)
                        (median, cost) = get3WayGeneric (Measure.stateTransitionThreewayDispersion (wideTCM charInfo)) a b c
                    in  (mempty, median, mempty, cost)
            HugeSeq →
                let (a, b, c) = addGapsToChildren (hugeGapped p1p2cN) (hugeGapped p1p2)
                    (median, cost) = get3WayGeneric (Measure.stateTransitionThreewayDispersion (hugeTCM charInfo)) a b c
                in  (mempty, mempty, median, cost)
            t → error $ "Unrecognized character type: " <> show t

        (median2Slim, median2Wide, median2Huge, cost2) = case localCharType of
            t
                | t `elem` [SlimSeq, NucSeq] →
        ...
        (median3Slim, median3Wide, median3Huge, cost3) = case localCharType of
            t
                | t `elem` [SlimSeq, NucSeq] →
        ...
        (medianBestSlim, medianBestWide, medianBestHuge)
            | cost1 == minCost = (median1Slim, median1Wide, median1Huge)
            | cost2 == minCost = (median2Slim, median2Wide, median2Huge)
            | otherwise = (median3Slim, median3Wide, median3Huge)
    in  case localCharType of -- set for correct data type
