    ...
    if G.isEmpty inGraph
        then error ("Empty Graph")
            ...
            let topPart = "digraph G {\n\trankdir = LR; edge [colorscheme=spectral11]; node [shape = rect];\n"
                ...
                endPart = "}\n"
            in  topPart <> nodeList <> edgeList <> endPart
    where
        makeEdgeString (a, b) = (show a) <> " -> " <> (show b) <> ";\n"
...
hasDuplicateEdgesNub inGraph =
    if isEmpty inGraph
        then False
            ...
            in  not (length edgeList == length uniqueEdgeList)
...
hasDuplicateEdge inGraph =
    if isEmpty inGraph
        then False
            ...
            in  -- dupEdges = (fmap toEdge $ labEdges inGraph) L.\\ (L.nub $ fmap toEdge $ labEdges  inGraph)
...
hasAllNetChildren inGraph inNode =
    ...
    in  length children == length childVertNodes
        ...
        then Nothing
            ...
            in  if null netVertexList
                    then Just inGraph
                        ...
                        if null chainedNodeList
                            then Just inGraph
                            else
                                if null fixableChainedEdgeList
                                    then
                                        let warningString =
                                                if showWarning
                                                    then "Warning: Unfixable chained network nodes (both parent and child nodes are indegree > 1). Skipping graph"
                                                    else ""
                                        in  traceNoLF
                                                warningString
                                                Nothing
                                        ...
                                        let warningString =
                                                if showWarning
                                                    then
                                                        "Warning: Chained network nodes (both parent and child nodes are indegree > 1), removing edges to tree node parents (this may affect graph cost)\n"
                                                    else ""
                                        in  traceNoLF
                                                warningString
...
updateNodeLabel inGraph inNodeIndex newLabel =
    if isEmpty inGraph
        then error "Empty graph in sisterLabNodes"
        else
            let incidentEdges = (inn inGraph inNodeIndex) <> (out inGraph inNodeIndex)
            in  insEdges incidentEdges $ insNode (inNodeIndex, newLabel) $ delNode inNodeIndex inGraph
...
sisterLabNodes inGraph inNode =
    if isEmpty inGraph
        then error "Empty graph in sisterLabNodes"
            ...
            in  -- shoudl not need nub for phylogenetic graphs but whatever
...
grandParents inGraph inNode =
    ...
    in  concatMap (parents inGraph) nodeParents
...
sharedGrandParents inGraph inNode =
    if isEmpty inGraph
        then error "Empty gaph in sharedGrandParents"
        else
            if isRoot inGraph inNode
                then False
                else
                    let parentList = parents inGraph inNode
...
labParents inGraph inNode =
    ...
    in  if hasNothing
            then error "Unlabeled nodes in labParents"
...
componentGraphs inGraph =
    if isEmpty inGraph
        then []
            ...
            in  if length componentNodeListList == 1
                    then [inGraph]
...
labelNode inGraph inNode =
    if isEmpty inGraph
        then error "Empty graph for label source"
            ...
            in  if isNothing label
                    then error ("No label for node " <> show inNode)
                    else (inNode, fromJust label)
    ...
    if null edgeList
        then False
            ...
            if isNetworkLabEdge inGraph (head edgeList)
                then True
                else hasNetworkEdgeList inGraph (drop 1edgeList)
...
isTree inGraph =
    if G.isEmpty inGraph
        then error "Empty graph in isTree"
        else
...
splitVertexList inGraph =
    if G.isEmpty inGraph
        then ([], [], [], [])
            ...
            in  (rootList, leafList, treeVertexList, networkVertexList)
...
pathToRoot inGraph inNode =
    if G.isEmpty inGraph
        then error "Empty graph in pathToRoot"
...
pathToRoot' inGraph inNodeList curNodeList curEdgeList =
    if null inNodeList
        then (reverse curNodeList, reverse curEdgeList)
        else
            let inNode = head inNodeList
            in  -- root would already be inlist of nodes visited
                if isRoot inGraph (fst inNode)
                    then pathToRoot' inGraph (drop 1inNodeList) curNodeList curEdgeList
                    else
                        ...
                        in  pathToRoot' inGraph (inLabNodes <> drop 1inNodeList) (inLabNodes <> curNodeList) (inLEdges <> curEdgeList)
        ...
        then (reverse curNodeList, reverse curEdgeList)
            ...
            in  -- root would already be inlist of nodes visited
                if (fst inNode) == (fst endNode)
                    then postOrderPathToNode' inGraph endNode (drop 1inNodeList) curNodeList curEdgeList
                    else
                        if isRoot inGraph (fst inNode)
                            then
                                error
                                    ( "postOrderPathToNode hit root before end node.  Root index  "
                                        <> (show $ fst inNode)
                                        <> " edges "
                                        <> (show $ fmap toEdge curEdgeList)
                                    )
                            else
                                let inLEdges = filter (`notElem` curEdgeList) $ inn inGraph (fst inNode)
                                    inNodes = filter (`notElem` (fmap fst curNodeList)) $ fmap fst3 inLEdges
                                    inLabNodes = zip inNodes (fmap (fromJust . lab inGraph) inNodes)
                                in  postOrderPathToNode' inGraph endNode (inLabNodes <> drop 1inNodeList) (inLabNodes <> curNodeList) (inLEdges <> curEdgeList)
...
nodesAndEdgesBefore' inGraph curResults@(curNodes, curEdges) inNodeList
    ...
    | otherwise =
        ...
        in  if Nothing `elem` labelMaybeList
                ...
                else nodesAndEdgesBefore' inGraph (intoLabNodeList <> curNodes, intoEdgeList <> curEdges) (intoLabNodeList <> drop 1inNodeList)
...
getEdgeListAfter (inGraph, inNode) =
    if isEmpty inGraph
        then error "Input Graph is empty in getEdgeListAfter"
...
nodesAndEdgesAfter' inGraph curResults@(curNodes, curEdges) inNodeList
    ...
    | otherwise =
        ...
        in  if Nothing `elem` labelMaybeList
                then
                    error
                        ( "Empty node label in nodesAndEdgesAfter"
                            <> show (filter ((== Nothing) . snd) $ zip fromNodeList labelMaybeList)
                            <> " "
                            <> (show fromNodeList)
                            <> "\n"
                            <> (prettyIndices inGraph)
                    ...
                    in  nodesAndEdgesAfter' inGraph (fromLabNodeList <> curNodes, fromEdgeList <> curEdges) (fromLabNodeList <> drop 1inNodeList)
...
coevalNodePairs inGraph =
    ...
    where
        makePairs a =
            if length a /= 2
                then error ("Not two parents for coevalNodePairs")
                else (head a, a !! 1)
...
contractRootOut1Edge inGraph =
    if G.isEmpty inGraph
        then G.empty
            ...
            in  -- trace ("CRO1E :" <> (show (inOutDeg, out1RootList, out2RootList))) (
                if null out1RootList
                    then inGraph
                    else
                        if length out2RootList > 1
                            then error ("Multiple roots in graph in contractRootOut1Edge: " <> (show $ length out2RootList))
                            else
                                if null out2RootList
                                    then
                                        ...
                                        in  -- trace ("Removing drop 1edge root :" <> (show $ snd3 $ head $ out inGraph ((fst . fst3) rootVertex)))
                                        ...
                                        in  -- trace ("Removing extra edge root :" <> (show $ (root02EdgeToDelete, newEdgeFrom02Root, newEdgeFrom01Root)))
...
contractIn1Out1Edges inGraph =
    if G.isEmpty inGraph
        then G.empty
            ...
            in  -- trace ("vertex 11:" <> show degree11VertexList) (
                if null degree11VertexList
                    then inGraph
                    else
                        ...
                        in  -- newGraph = insEdge newEdgeToAdd $ delLNode nodeToDelete inGraph
...
reindexNodes inNodeIndex curList nodeList =
    if null nodeList
        then reverse curList
        else
            let firstNode@(index, label) = head nodeList
            in  if index < inNodeIndex
                    then reindexNodes inNodeIndex (firstNode : curList) (drop 1nodeList)
                    else
                        if index == inNodeIndex
                            then reindexNodes inNodeIndex curList (drop 1nodeList)
                            else reindexNodes inNodeIndex ((index - 1, label) : curList) (drop 1nodeList)
        ...
        then curList
            ...
            in  -- incident on node to be deleted
                if a == inNodeIndex || b == inNodeIndex
                    then reindexEdges inNodeIndex curList (drop 1edgeList)
                    else -- reindexed edge added in
                        reindexEdges inNodeIndex ((a', b', c) : curList) (drop 1edgeList)
...
removeNonLeafOut0NodesAfterRoot' inGraph =
    if isEmpty inGraph
        then empty
            ...
            in  removeNonLeafOut0Nodes leafList inGraph
...
removeNonLeafOut0NodesAfterRoot inGraph =
    if isEmpty inGraph
        then empty
            ...
            in  -- trace ("RNLOoN: " <> (show (rootIndex, fmap fst zeroOutNodeList)) ) -- <> "\n" <> (prettyIndices inGraph)) (
                if null zeroOutNodeList
                    then inGraph
                    else
                        let newGraph = delNodes (fmap fst zeroOutNodeList) inGraph
                        in  removeNonLeafOut0NodesAfterRoot $ reindexGraph newGraph
...
removeNonLeafOut0Nodes leafList inGraph =
    if null leafList
        then error "Null leaf list in removeNonLeafOut0Nodes"
        else
            if isEmpty inGraph
                then empty
                else
                    ...
                    in  if null zeroOutNodeList
                            then inGraph
                            else
                                let newGraph = delNodes (fmap fst zeroOutNodeList) inGraph
                                in  removeNonLeafOut0Nodes leafList newGraph
...
reindexGraph inGraph =
    if isEmpty inGraph
        then empty
            ...
            in  mkGraph newNodeList newEdgeList
    where
        makeNewNode indexMap (a, b) = (fromJust $ MAP.lookup a indexMap, b)
...
getBackEdges _ [] = []
getBackEdges v ls = map head (filter (elem (v, 0)) (drop 1ls))
...
lowTree (B (v, n, bcks) trs) = Brc (v, n, lowv) ts
    where
        lowv = min (minbckEdge n bcks) lowChild
...
testEdge fullGraph candidateEdge@(e, u, _) =
    ...
    in  [candidateEdge | isNothing foundU]
...
getCoevalConstraintEdges inGraph inNode =
    if isEmpty inGraph
        then error "Empty input graph in getCoevalConstraintEdges"
        else
...
meetsAllCoevalConstraintsNodes constraintList edge1@(u, v, _) edge2@(u', v', _) =
    if null constraintList
        then True
            ...
            in  -- checks if edge nodes would stradle existing coeveal nodes
                if (u `elem` aNodesAfter) && (u' `elem` bNodesBefore)
                    then False
                    else
                        if (u' `elem` aNodesAfter) && (u `elem` bNodesBefore)
                            then False
                            else
                                if (v `elem` aNodesAfter) && (v' `elem` bNodesBefore)
                                    then False
                                    else
                                        if (v' `elem` aNodesAfter) && (v `elem` bNodesBefore)
                                            then False
                                            else
                                                if (u `elem` aNodesBefore) && (u' `elem` bNodesAfter)
                                                    then False
                                                    else
                                                        if (u' `elem` aNodesBefore) && (u `elem` bNodesAfter)
                                                            then False
                                                                ...
                                                                if (v `elem` aNodesBefore) && (v' `elem` bNodesAfter)
                                                                    then False
                                                                        ...
                                                                        if (v' `elem` aNodesBefore) && (v `elem` bNodesAfter)
                                                                            then False
                                                                            else meetsAllCoevalConstraintsNodes (drop 1constraintList) edge1 edge2
...
meetsAllCoevalConstraintsEdges constraintList edge1 edge2 =
    if null constraintList
        then True
            ...
            in  if edge1 `elem` beforeList && edge2 `elem` afterList
                    then False
                    else
                        if edge2 `elem` beforeList && edge1 `elem` afterList
                            then False
                            else meetsAllCoevalConstraintsEdges (drop 1constraintList) edge1 edge2
...
notMatchEdgeIndices unlabeledEdegList labelledEdge =
    if toEdge labelledEdge `elem` unlabeledEdegList
        then False
        else True
...
addBeforeAfterToPair inGraph (a, b) =
    if isEmpty inGraph
        then error "Empty graph in addBeforeAfterToPair"
            ...
            in  (a, b, aNodesBefore, bNodesBefore, aNodesAfter, bNodesAfter)
        ...
        then []
            ...
            if null inNodePairList
                then []
                    ...
                    in  -- trace ("GETRT: " <> (show inNodePairList)) (
                        -- condition if a before a' then b before b' to be ok
                        if (a' `elem` aNodesAfter) && (b' `elem` bNodesBefore)
                            then
                                ...
                                in  -- trace ("\tRemoving network edge due to time consistancy: " <> (show edgeToRemove))
                                    ...
                                    edgeToRemove : getEdgesToRemoveForTime inGraph (drop 1inNodePairList)
                            else
                                                ...
                                                if (b' `elem` aNodesBefore) && (a' `elem` bNodesAfter)
                                                    then
                                                        ...
                                                        in  -- trace ("GERT Edges1:" <> (show edgeToRemove) <> " " <> (show $ fmap toEdge $ out inGraph $ fst b'))
                                                            -- trace ("\tRemoving network edge due to time consistancy : " <> (show edgeToRemove))
                                                            edgeToRemove : getEdgesToRemoveForTime inGraph (drop 1inNodePairList)
                                                    else getEdgesToRemoveForTime inGraph (drop 1inNodePairList)
    ...
    if isEmpty inGraph
        then error ("Empty graph in getEdgeSplitList")
            ...
            let origNumComponents = noComponents inGraph
                ...
                bridgeList' =
                    filter ((not . isRoot inGraph) . fst3) $
                        filter ((not . isNetworkEdge inGraph) . toEdge) $
                            filter ((not . isOutDeg1Node inGraph) . fst3) bridgeList
            in  -- trace ("BridgeList" <> (show $ fmap toEdge bridgeList') <> "\nGraph\n" <> (prettyIndices inGraph))
                bridgeList'
...
splitGraphOnEdge inGraph (e, v, l) =
    if isEmpty inGraph
        then error "Empty graph in splitGraphOnEdge"
        else
            if (length $ getRoots inGraph) /= 1
                then error ("Incorrect number roots in splitGraphOnEdge--must be 1: " <> (show $ fmap fst $ getRoots inGraph))
                else
                    ...
                    in  if length childrenENode /= 1
                            then error ("Incorrect number of children of edge to split--must be 1: " <> (show ((e, v), childrenENode)))
                            else
                                if length parentsENode /= 1
                                    then error ("Incorrect number of parents of edge to split--must be 1: " <> (show ((e, v), parentsENode)))
                                    else -- trace ("SGE:" <> (show (childrenENode, parentsENode, newEdge, edgesToDelete)))
...
joinGraphOnEdge inGraph (x, y, l) parentofPrunedSubGraph =
    if isEmpty inGraph
        then error ("Empty graph in joinGraphOnEdge")
        else
            let edgeToCreate0 = (x, parentofPrunedSubGraph, l)
                edgeToCreate1 = (parentofPrunedSubGraph, y, l)
            in  -- edgeToCreate2 = (parentofPrunedSubGraph, graphToJoinRoot, l)
    ...
    if null netNodeList
        then []
            ...
            in  if null sameGrandParentList
                    then getSisterSisterEdgeByNetVertex inGraph (drop 1netNodeList)
                    else -- trace ("Found sister-sister")
                        (toEdge $ head $ inn inGraph firstNode) : getSisterSisterEdgeByNetVertex inGraph (drop 1netNodeList)
...
concurrentViolatePair inGraph (node1, node2) =
    if isEmpty inGraph
        then error "Empty graph in concurrentViolatePair"
        else
            let (nodesBeforeFirst, _) = nodesAndEdgesBefore inGraph [node1]
                (nodesBeforeSecond, _) = nodesAndEdgesBefore inGraph [node2]
            in  if node2 `elem` nodesBeforeFirst
                    then [(node1, node2)]
                    else
                        if node1 `elem` nodesBeforeSecond
                            then [(node1, node2)]
                            else []
...
mergeConcurrentNodeLists inListList currentListList =
    if null inListList
        then -- trace ("MCNL:" <> (show $ fmap (fmap fst) currentListList))
            currentListList
        else -- first case

            if null currentListList
                then mergeConcurrentNodeLists (drop 1inListList) [head inListList]
                else
...
sortEdgeListByDistance inGraph eNodeList vNodeList =
    if isEmpty inGraph
        then error ("Empty graph in edgeListByDistance")
            ...
            if (null eNodeList && null vNodeList)
                then []
                else -- get edges 'in' to eNodeList
                    ...
                    in  inEdgeList <> outEdgeList <> (sortEdgeListByDistance inGraph newENodeList newVNodeList)
...
switchRootTree newRootIndex inGraph =
    if isEmpty inGraph
        then empty
            ...
            in  -- not a directed tree
                if length rootList /= 1
                    then error ("Graph input to switchRootTree is not a tree--not single root:" <> (show rootList))
                    else -- same root

                        if newRootIndex == (fst $ head rootList)
                            then inGraph
                            else -- create new edges and delete the old ones

                                let newEdgesToAdd = fmap (flipVertices (fst $ head rootList) newRootIndex) (newRootCurInEdges <> newRootCurOutEdges <> oldRootEdges)
                                in  insEdges newEdgesToAdd $ delLEdges (newRootCurInEdges <> newRootCurOutEdges <> oldRootEdges) inGraph
...
flipVertices a b (u, v, l) =
    ...
    in  -- trace (show (u,v,l) <> "->" <> show (newU, newV, l))
        (newU, newV, l)
...
rerootTree rerootIndex inGraph =
    ...
    if isEmpty inGraph
        then inGraph
            ...
            in  -- (_, inNewRoot, outNewRoot) = getInOutDeg inGraph (labelNode inGraph rerootIndex)

                -- rerooting on root so no indegree edges
                -- this for wagner build reroots where can try to reroot on leaf not yet added
                if null $ inn inGraph rerootIndex
                    then inGraph -- error ("Rerooting on indegree 0 node " <> (show rerootIndex) <> "\n" <> prettyIndices inGraph) -- empty
                    else
                        if null componentWithOutgroup
                            then inGraph -- error ("Error rooting wierdness in rerootTree " <> (show rerootIndex) <> "\n" <> prettyIndices inGraph) -- empty
                            else -- check if new outtaxon has a parent--shouldn't happen-but could if its an internal node reroot

                                if null parentNewRootList || (True `elem` parentRootList)
                                    then inGraph
                                    else
                                        ( if null componentWithOutgroup
                                            then error ("Outgroup index " <> show rerootIndex <> " not found in graph")
                                            else -- trace ("RRT: " <> (show (rerootIndex, inNewRoot, outNewRoot))) (
                                            -- reroot component with new outtaxon
                                                ...
                                                in  if numRoots == 0
                                                        then error ("No root in rerootTree: Attempting to reroot on edge to node " <> (show rerootIndex) <> "\n" <> prettyIndices inGraph) -- empty
                                                        else -- check if outgroup in a multirooted component
                                                        -- if wagner build this is ok

                                                            if numRoots > 1
                                                                then inGraph -- error ("Error: Attempting to reroot multi-rooted component") -- inGraph
                                                                else -- reroot graph safely automatically will only affect the component with the outgroup
                                                                ...
                                                                -- trace ("Moving root from " <> (show orginalRoot) <> " to " <>  (show rerootIndex)) (
                                                                    ...
                                                                    let leftChildEdge = (orginalRoot, rerootIndex, edgeLabel $ head originalRootEdges)
                                                                        ...
                                                                        newEdgeOnOldRoot =
                                                                            if (length originalRootEdges) /= 2
                                                                                then
                                                                                    error
                                                                                        ( "Number of root out edges /= 2 in rerootGraph: "
                                                                                            <> (show $ length originalRootEdges)
                                                                                            <> " root index: "
                                                                                            <> (show (orginalRoot, rerootIndex))
                                                                                            <> "\nGraph:\n"
                                                                                            <> (prettyIndices inGraph)
                                                                                        )
...
rerootDisplayTree orginalRootIndex rerootIndex inGraph =
    ...
    if isEmpty inGraph
        then inGraph
            ...
            in  -- these here for checking in1out1 edges on reroot
                ...
                if (inNewRoot == 1) && (outNewRoot == 1)
                    then inGraph
                    else -- else if parentNewRootIn1Out1 || childNewRootIn1Out1 then inGraph
                    ...
                    -- this for wagner build reroots where can try to reroot on leaf not yet added
                        ...
                        if null $ inn inGraph rerootIndex
                            then inGraph -- error ("Rerooting on indegree 0 node " <> (show rerootIndex) <> "\n" <> prettyIndices inGraph) -- empty
                                ...
                                if null parentNewRootList || (True `elem` parentRootList)
                                    then inGraph
                                    else -- else if null componentWithOutgroup then error ("Outgroup index " <> show rerootIndex <> " not found in graph")
                                    ...
                                    -- reroot component with new outtaxon
                                        ...
                                        let -- componentWithNewOutgroup = snd $ head componentWithOutgroup
                                            ...
                                            originalRootEdges = (out inGraph orginalRoot)
                                        in  -- reroot graph safely automatically will only affect the component with the outgroup
                                            -- delete old root edge and create two new edges from oringal root node.
                                            -- keep orignl root node and delte/crete new edges when they are encounterd
                                            -- trace ("Moving root from " <> (show orginalRoot) <> " to " <>  (show rerootIndex)) (
                                            let leftChildEdge = (orginalRoot, rerootIndex, edgeLabel $ head originalRootEdges)
                                                ...
                                                newEdgeOnOldRoot =
                                                    if (length originalRootEdges) /= 2
                                                        then (snd3 $ head originalRootEdges, snd3 $ last originalRootEdges, thd3 $ head originalRootEdges)
                                                        else {-
                                            ...
                                            in  -- this erros for malformed newEdge on old root
                                                if (length originalRootEdges) /= 2
                                                    then inGraph
                                                    else -- checks that new root has two edges

                                                        if (length $ out newGraph' orginalRootIndex) /= 2
                                                            then inGraph
                                                            else newGraph'
    ...
    if null inEdgeList
        then inGraph
            ...
            in  -- trace ("PTFE: flipped " <> (show $ fmap toEdge flippedEdges)) (
                ...
                if null childEdges
                    then preTraverseAndFlipEdgesTree rootIndex (drop 1inEdgeList) inGraph
                    else -- edge needs to be reversed to follow through its children from a new graph
                        preTraverseAndFlipEdgesTree rootIndex (flippedEdges <> (drop 1inEdgeList)) newGraph
        ...
        then inGraph
            ...
            in  -- trace ("PTFE: flipped " <> (show $ fmap toEdge flippedEdges)) (
                ...
                if null childEdges || null edgesToFlip
                    then preTraverseAndFlipEdges (drop 1inEdgelist) inGraph
                    else -- edge needs to be reversed to follow through its children from a new graph
                        preTraverseAndFlipEdges (flippedEdges <> (drop 1inEdgelist)) newGraph
...
-}
generateDisplayTreesRandom ∷ ∀ a b. (Show a, Show b, Eq a, Eq b, NFData a, NFData b) ⇒ Int → Gr a b → PhyG [Gr a b]
...
randomlyResolveGraphToTree inGraph
    ...
    | otherwise =
        ...
        in  do
                edgesToDelete ← fmap concat . traverse chooseOneDumpRest $ fmap toEdge <$> inEdgeListByVertex
...
generateDisplayTrees contractEdges inGraph =
    if isTree inGraph
        then [inGraph]
            ...
            let (_, leafList, _, _) = splitVertexList inGraph
            in  generateDisplayTrees' contractEdges leafList [inGraph] []
            ...
            in  if isEmpty firstGraph
                    then []
                        ...
                        in  if null inNetEdgeList
                                then generateDisplayTrees' contractEdges leafList (drop 1curGraphList) (firstGraph : treeList)
                                else
                                    let newGraphList = splitGraphListFromNode inNetEdgeList [firstGraph]
                                    in  generateDisplayTrees' contractEdges leafList (newGraphList <> (drop 1curGraphList)) treeList
...
splitGraphListFromNode inEdgeListList inGraphList =
    if null inEdgeListList
        then inGraphList
        else
            if null inGraphList
                then error "Empty graph lits in splitGraphListFromNode"
                else
                    ...
                    in  splitGraphListFromNode (drop 1inEdgeListList) newGraphList
...
deleteEdgesCreateGraphs netEdgeIndexPairList counter inGraph =
    if isEmpty inGraph
        then error "Empty graph in  " deleteEdgesCreateGraphs
        else
            if null netEdgeIndexPairList
                then []
                else
    ...
    if null firstList
        then []
            ...
            in  if firstEdge `L.elem` secondList
                    then undirectedEdgeMinus (drop 1firstList) secondList
                    else
                        if (b, a) `L.elem` secondList
                            then undirectedEdgeMinus (drop 1firstList) secondList
                            else firstEdge : undirectedEdgeMinus (drop 1firstList) secondList
...
remakeGraph inGraph =
    if isEmpty inGraph
        then inGraph
        else mkGraph (labNodes inGraph) (labEdges inGraph)
