...
getAdamsIIPair inGraphVectA inGraphVectB =
    ...
    in  if not sameLeafSet
            then errorWithoutStackTrace ("Leaf sets of input graphs do not match" <> show leafSets)
            else -- return processed when have all nodes
                ...
                in  genPhyNet2PhyloGraphVect allAdamsNodes
...
makeAdamsII leafNodeList inFGList
    ...
    | otherwise =
        ...
        in  do
                ...
                if not allTrees
                    then errorWithoutStackTrace ("Input graphs are not all trees in makeAdamsII: " <> show allTreesList)
                    else
                        if not (leafSetConstant [] inFGList)
                            then errorWithoutStackTrace "Input leaf sets not constant in makeAdamsII"
                            else
...
fgl2PGVNode inGraph ((index, inLabel), childList, parentList) =
    ...
    in  if null parentList
            then (label, childList, parentList, Root)
            else
                if null childList
                    then (label, childList, parentList, Leaf)
                    else
                        if length parentList == 1
                            then (label, childList, parentList, Reconciliation.Adams.Tree)
                            else
                                if length parentList > 1
                                    then (label, childList, parentList, Network)
...
fgl2PGV inGraph =
    if G.isEmpty inGraph
        then nullGraphVect
            ...
            in  (V.fromList pgvNodeList, V.fromList pgvEdgeList)
...
vertex2Node counter inVertexVect =
    if V.null inVertexVect
        then []
        else
            let (label, _, _, _) = V.head inVertexVect
            in  (counter, label) : vertex2Node (counter + 1) (V.tail inVertexVect)
...
pgv2FGL (inVertexVect, inEdgeVect) =
    ...
    in  G.mkGraph fglNodes (V.toList fglEdges)
...
getLeafLabelListFGL inGraph =
    if G.isEmpty inGraph
        then error "Empty graph in getLeafLabelListFGL"
        else
...
leafSetConstant leafList inFGLList
    ...
    | null leafList =
        ...
        in  leafSetConstant firstLeaves (tail inFGLList)
    | otherwise =
        let thisGraph = head inFGLList
            theseLeaves = getLeafLabelListFGL thisGraph
        in  theseLeaves == leafList && leafSetConstant leafList (tail inFGLList)
...
isTree inGraph =
    not (G.isEmpty inGraph)
        && ( let nodeIndegrees = G.indeg inGraph <$> G.nodes inGraph
             ...
             in  not ((((length rootNodes /= 1) || (maxIndegree > 1)) || (length leafLabels /= length uniqueLeafLabels)) || not (null selfList))
           )
...
getRootNamesFromGenPhyNet inNet =
    if null inNet
        then []
            ...
            in  if vertType == Root
                    then name : getRootNamesFromGenPhyNet (tail inNet)
                    else getRootNamesFromGenPhyNet (tail inNet)
...
getNonLeafNonRootNamesFromGenPhyNet inNet =
    if null inNet
        then []
            ...
            let (name, desc, anc) = head inNet
                vertType = getVertType (length desc) (length anc)
            in  if (vertType /= Leaf) && (vertType /= Root)
                    then name : getNonLeafNonRootNamesFromGenPhyNet (tail inNet)
                    else getNonLeafNonRootNamesFromGenPhyNet (tail inNet)
...
getLeafNamesFromGenPhyNet inNet =
    if null inNet
        then []
            ...
            let (name, desc, anc) = head inNet
            in  if getVertType (length desc) (length anc) == Leaf
                    then name : getLeafNamesFromGenPhyNet (tail inNet)
                    else getLeafNamesFromGenPhyNet (tail inNet)
...
getVertNum nameList vertexNameList =
    if null vertexNameList
        then []
        else
            let firstVertName = head vertexNameList
                vertNum = L.elemIndex firstVertName nameList
            in  if isNothing vertNum
                    then error ("Error in vertex name index: " <> show firstVertName <> " in " <> show nameList)
                    else fromJust vertNum : getVertNum nameList (tail vertexNameList)
...
oldIndex2New inVertexVect nameList =
    if V.null inVertexVect
        then []
            ...
            in  (fromJust vertNum, curVert) : oldIndex2New (V.tail inVertexVect) nameList
...
getNamesFromGenPhyNet inNet =
    L.sort (getLeafNamesFromGenPhyNet inNet)
        <> getNonLeafNonRootNamesFromGenPhyNet inNet
        <> getRootNamesFromGenPhyNet inNet
...
getShortestList bestList lengthBestList inListList =
    if null inListList
        then bestList
            ...
            in  if lengthCurList < lengthBestList
                    then getShortestList curList lengthCurList (tail inListList)
...
getSplitList curLUB placedTaxa (potentialVerts, vertLeafSet) =
    if null curLUB
        then error "Null LUB in getSplitList"
        else
...
genPhyNet2PhyloGraphVect inGenPhyNet =
    if null inGenPhyNet
        then error "Null GenPhyNet in genPhyNet2PhyloGraphVect"
        else
...
makeAdamsNodes inAdamsTree parentName inLUBList placedTaxa bothLeafLists =
    -- inTreeVertexLists vertexLeafSetList =
    if null inLUBList
        then inAdamsTree
        else
            let curLUB = head inLUBList
            in  if length curLUB == 1 -- make nodes since done
                    then
                        ...
                        in  makeAdamsNodes
                                ...
                                parentName
                                (tail inLUBList)
                                (head curLUB : placedTaxa)
                                bothLeafLists -- inTreeVertexLists vertexLeafSetList
                    else
                        if length curLUB == 2
                            then
                                ...
                                in  -- trace ("New LUBs " <> show newLUB <> " newNode " <> show newNode)
                                    makeAdamsNodes
                                        (newNode : inAdamsTree)
                                        (lub2TreeRep curLUB)
                                        (tail inLUBList)
...
getLeafSetFromNodeName inVertex =
    ...
    in  if null nodeName
            then error "Null node name in getLeafSetFromNodeName"
            else
                let rawList = map (replaceChar ['(', ')', ','] ' ') nodeName
                in  L.sort $ words rawList -- this sort required
...
getDecendantLeafList inVertexList inGraphVect =
    if null inVertexList
        then []
            ...
            in  if vertType == Leaf
                    then curVertName : getDecendantLeafList (tail inVertexList) inGraphVect
                    else
...
getSplitLeafListList descListList inGraphVectList
    ...
    | otherwise =
        ...
        in  getSplitLeafList curIntList curGraphVectList
...
lub2 s1 s2
    ...
    | otherwise =
        ...
        in  intersectFirst <> lub2 (tail s1) s2
...
leafSetFromVertexVect inSet inVerts =
    if V.null inVerts
        then inSet
            ...
            in  if curType == Leaf
                    then leafSetFromVertexVect (Set.insert curName inSet) (V.tail inVerts)
                    else leafSetFromVertexVect inSet (V.tail inVerts)
...
getLeafSet inGraphVect =
    ...
    in  leafSetFromVertexVect Set.empty inVerts
...
setEqual firstSet secondSet =
    ...
    in  Set.empty == Set.difference combinedElem sameElem
...
getAndCheckLeafSets inGraphs =
    if null inGraphs
        then error "Empty graph list in getAndCheckLeafSets"
        else
...
findRoot index inGraph =
    ...
    in  if index < V.length vertexVect
            then
                let (_, _, _, vertexType) = vertexVect V.! index
                in  if vertexType == Root
                        then (index, vertexVect V.! index)
                        else findRoot (index + 1) inGraph
